...
+
You should see the following code in the code panel of the Playground page. Copy at any point down to `.render();`:
+
[source,javascript]
----
...
. Click *Run* to see the `LiveboardEmbed` component load with the new options.
. Remove `//` before the `hideLiveboardHeader: true` property to enable this setting.
. Click *Run* to see the updated results.
...
To override any of the user's locale settings:
...
. Click *Run* and try some of the menu system after the Liveboard loads:
...
If your implementation requires hiding specific elements, let ThoughtSpot know the element for which you need a configuration option by submitting an idea to the link:https://community.thoughtspot.com/s/ideas[ThoughtSpot Community^].
...
----
+
Notice the format is the selector as the __key__, then an object block containing individual key-value pairs for the properties. Because the selector is an object key, but all `@font-face` declarations start the same way, we add a unique CSS comment at the beginning to allow for multiple `@font-face` declarations.
...
In the Playground, click the checkbox for `Modify available actions`. You'll see the following code in the code editor:
...
ThoughtSpot allows you to add new items to the menu system, called *xref:custom-actions.adoc[custom actions]*, either to the *More* options menu on a given visualization of a Liveboard or the context menu that appears when a single point is right-clicked.
