...
Next, we'll find the tag to apply:
...
Next let's add the logic to create the tag if none is found with that name.

You'll notice that the autocomplete for the `TSRestApiV2.tags_create()` method shows defined arguments rather than a generic `request` argument. 
