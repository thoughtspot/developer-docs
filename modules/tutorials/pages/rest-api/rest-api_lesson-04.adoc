...
Modern JavaScript uses the link:https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise[Promise] concept to help with structuring asynchronous code.
...
If you are using the Visual Embed SDK, it is simplest to keep all ThoughtSpot related code within the block where you import the Visual Embed SDK module.
...
== 02 - Using fetch() and tokenizedFetch() to issue REST API requests
...
`tokenizedFetch()` is able to access the current *Bearer Token* used by the SDK when making requests, while using the exact syntax as the standard `fetch()` function:
...
There is a link:https://github.com/thoughtspot/rest-api-sdk/tree/release/sdks/typescript[TypeScript SDK] that can be used when working in Node.js or other environments where TypeScript is available. 
