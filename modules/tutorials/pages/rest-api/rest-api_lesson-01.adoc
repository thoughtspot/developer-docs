...
Every instance of ThoughtSpot has a REST API V2.0 Playground which is both the *specification* of the API and an interactive system for building and issuing API commands. image:images/tutorials/rest-api/playground-leftside-menu.png[Playground Left Side Menu, width=225, height=550, float=right, align=right] The Playground is organized into topics, which compose the expandable areas of the lefthand side menu: 
...
xref:rest-api_lesson-02.adoc[Next: 02 - Simple Python Implementation of V2.0 REST API >]
