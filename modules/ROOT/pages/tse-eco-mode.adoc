...
Additionally, support for Orgs tokens to wake up the cluster has been added. When an org-specific token is used (for example, via trusted authentication or API), it can trigger cluster wake-up, ensuring that org-level user sessions and activities are correctly recognized by idlesense logic.