...
| `storage_destination` | Configuration for storage destination. Include the following attributes if you want to add destination storage to store the attachments delivered via a Webhook.
...
If the webhook creation is successful, the API returns 204 response code.
...
[NOTE]
====
When you delete a webhook with S3 storage, the webhook endpoint is removed and any events or workflows configured to use that webhook will no longer be able to deliver payloads. The files already stored in S3 are not deleted as part of webhook deletion; only the delivery mechanism will be  removed.
====
