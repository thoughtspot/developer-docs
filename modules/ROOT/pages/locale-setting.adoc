...
The `locale` property defines the overall language of the app, and the default formats for numbers, dates and currencies To override the default number, date, and currency format set by the `locale` parameter, define the following attributes in `init` function as shown in this example:
...
ThoughtSpot application users can set locale on the Profile settings page. To open profile settings page, click the user icon on the top navigation bar and then edit your locale preference.
...
Use the following REST API v1 and v2 endpoints to change a user's locale preference.
