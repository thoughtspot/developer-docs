...
You can treat the REST API *token request* with any values where `persist: true` as a "update user_parameters" API endpoint, even if you don't use the token in the response.
...
Remember, the successful *request* for a token *updates* the stored `user_parameters` property of the user, rather than the first use of token.
