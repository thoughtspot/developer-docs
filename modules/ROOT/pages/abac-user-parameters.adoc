...
`sql_bool_op('{0} IN (SELECT DatabaseField FROM FullyQualfiedTable WHERE OtherField = {1})', WorksheetField , Param)`
...
The overall pattern is to use the Parameter value, sent in via ABAC, to retrieve a specific set of values set within the database, using whatever techniques are avaliable.
...
3. The "entitlement_session_id" is sent as an ABAC parameter as part of the ThouhghtSpot token request for the user
4. Worksheets that need these entitlements use the combination of pass-through function with parameter + formula + filter so that all queries in ThoughtSpot include a WHERE clause with the the sub-select
