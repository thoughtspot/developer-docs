...
You can now embed Collections and Liveboard schedule pages and set any of these application pages as the initial loading page using `Page.LiveboardSchedules` and `Page.Collections` enumeration values. For more information, see xref:set-default-page.adoc[Customizing default page and navigation path].
...
Users can now share saved Spotter conversations with other users and groups directly from the embedded Spotter interface. The Visual Embed SDK 1.52.0 adds the `Action.ShareSpotterConversation` action ID and the `disableSpotterConversationSharing` property to control this capability, along with host and embed events for programmatic sharing workflows.
+
For more information, see xref:customize-spotter-embed.adoc[Customize Spotter embed] and xref:spotter-agent-apis.adoc#_sharing_spotter_conversations[Sharing Spotter conversations].
...
To improve the initial load performance of large Liveboards, lazy loading is now enabled by default when `fullHeight` is set to `true` on `AppEmbed` or `LiveboardEmbed`. For more information, see xref:lazy-loading-fullheight.adoc[Lazy loading of Liveboard visualizations].
