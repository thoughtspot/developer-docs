...
////
===  Version control APIs for GIT workflows

=== New features and enhancements to the new Liveboard experience

////

=== Visual Embed Playground enhancements

The Visual Embed developer Playground now includes a *Try* button in the preview panel. The *Try* button is attached to an event handler, using which you can try out host events in the SDK to trigger an action in the embed mode.
...
[NOTE]
====
In the 9.2.0.cl release, ThoughtSpot does not support applying runtime Parameter overrides via Visual Embed SDK or REST API v2.0 endpoints.
====
...
To hide or disable the cross filter feature on an embedded instance, use the `Action.CrossFilter` and `Action.RemoveCrossFilter` parameters in the SDK. For more information, see xref:embed-actions.adoc[Show or hide menu items] and xref:embed-action-ref.adoc#liveboardv2-viz-actions[Action reference].
