...
The Answer Export API (`POST /api/rest/2.0/report/answer`) endpoint is now Generally available (GA) with the following enhancements:

* *Pinned Answer export*: Export a pinned visualization from a Liveboard directly by setting `type` to `PINNED` and passing the parent Liveboard GUID as `metadata_identifier`.
...
* *Scaling control*: Adjust chart element size in PNG exports using `scaling` (80-500%).
