...
You can now configure formula variables using Variable REST APIs, which can be used in RLS rules to dynamically assign security filters based on the Org, user, and data Model context. For example, you can define a formula variable for a region or department, assign different values for each Org or user, and reference it in RLS rules using the `ts_var` function.

Formula variables can also be used in the JWT token implementation for ABAC to enable secure and context-aware user access control by binding token-provided values to formulas used in RLS and data model filters.

For more information, see xref:variables.adoc[Variables] and xref:rls-rules.adoc[RLS rules] documentation.
