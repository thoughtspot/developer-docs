...
* xref:embed-spotter.adoc#configControls[Configuration controls in the SDK] to enable or disable data source, and show or hide sample questions
...
* Additional options in the xref:developer-playground.adoc#playground-spotter[Spotter Playground experience] to allow users to explore the configuration options.
...
If you are using REST APIs with basic authentication in your application environments, ThoughtSpot recommends using `secret_key` to generate an authentication token for your users. In ThoughtSpot embedding environments, we recommend that administrators enable Trusted Authentication on their instance and use `AuthType.TrustedAuthTokenCookieless` method in the SDK to authenticate users, and then enable MFA on your ThoughtSpot instances to stay security compliant and avoid breaking changes.

=== Liveboard enhancements

Compact Liveboard header::

Developers embedding ThoughtSpot Liveboards can now enable the Compact Liveboard [earlyAccess eaBackground]#Early Access# feature for their application users. When enabled, this feature optimizes Liveboard header space, moves the tabs panel to the top of the header, and maximize the visibility of the charts and tables on the Liveboard.
+
Compact Liveboard header is disabled by default. To enable compact header, set `isLiveboardCompactHeaderEnabled` to `true` in the SDK.
...
ThoughtSpot recommends migrating your Worksheet data to Models, because Worksheets are deprecated and will be removed from the ThoughtSpot in the 10.10.0.cl release.

=== ThoughtSpot Enterprise Org enablement
Starting with ThoughtSpot Cloud 10.6.0.cl release, the Orgs feature will be enabled by default on all ThoughtSpot Enterprise instances. Your environment will remain a single-tenant instance until you create Orgs. When the Orgs feature is enabled on your instance, you can switch to a single-tenant mode anytime by deleting all user-created Orgs and using only the default Primary Org.
