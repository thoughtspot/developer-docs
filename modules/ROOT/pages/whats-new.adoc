...
You can now customize a specific instance of text string in the ThoughtSpot UI using string IDs.
...
* link:https://github.com/thoughtspot/swift-embed-sdk[Swift Embed SDK, window=_blank] to embed ThoughtSpot content in iOS apps
* link:https://github.com/thoughtspot/android-embed-sdk[Android Embed SDK, window=_blank] to embed ThoughtSpot content in Android apps

For more information, see xref:mobile-embed.adoc[Embed ThoughtSpot in a mobile app].

=== Spotter embed components

The following Spotter embed components in the Visual Embed SDK are deprecated:
...
Impacted interfaces::
The new Spotter embed components are available for implementations using Visual Embed SDK v1.38.0 and later.
The old Spotter embed components such as `ConversationEmbed`, `ConversationViewConfig`, `BodylessConversation`, and `BodylessConversationViewConfig` are deprecated from Visual Embed SDK v1.38.0 onwards.
On ThoughtSpot instances with 10.9.0.cl, the Visual Embed Playground shows the old component names, which will be replaced with new names in the upcoming ThoughtSpot release.

Recommended action::

The Spotter embed component changes do not introduce any breaking changes to your existing Spotter embed implementation. Your Spotter embed implementation with the old component names will continue to work until further notice. ThoughtSpot recommends switching to the new names early on, so you have sufficient time to test your implementation with the new names.
...
You can now show or hide the following columns on the *Liveboards*, and *Answers* list pages.
