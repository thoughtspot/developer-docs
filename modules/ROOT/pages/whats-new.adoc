...
You can now configure a xref:webhooks-lb-schedule.adoc[webhook for Liveboard schedule events] [beta betaBackground]^Beta^ to automate notifications to external applications. This feature allows you to send Liveboard reports directly to a webhook endpoint, such as your internal marketing or incident management app, at a scheduled frequency. It  eliminates the need to manually process data from email notifications and gives you full control over how the data is handled in your downstream workflows.
