...
== Version 10.4.0.cl
=== Orgs enablement
Starting from the ThoughtSpot 10.4.0.cl release, the Orgs will be enabled by default on all new clusters . Your environment remains single-tenant until you create an Org. While Orgs are enabled, you can also delete all the created Orgs, and just use the Primary Org in the single-tenant mode.
For more information, see xref:multi-tenancy-intro.adoc[Orgs Enablement].
