...
The new navigation and homepage experience is now available on ThoughtSpot instances. To enable this feature on your instance, contact your ThoughtSpot administrator.

If enabled at the cluster level, the application interface switches to the New experience mode for all users. However, users can turn off the new experience on their *Profile* settings page and return to the classic experience mode. On embedding apps, the new navigation and homepage experience is disabled by default. To enable it, you must set the `modularHomeExperience` property to `true` in the `AppEmbed` component.
...
////
=== Support for Models [earlyAccess eaBackground]#Early Access#

Embedding application users can now use Models, an improved version of Worksheets as data objects and sources for building their Answers and Liveboards. Users with data management privileges can create a model on the Data Workspace page. For more information, see link:https://docs.thoughtspot.com/cloud/latest/models[Models, window=_blank].


=== Attribute-Based tokens [earlyAccess eaBackground]#Early Access#
