...
The WYSIWYG Liveboard PDF export experience is now generally available for ThoughtSpot Embedded instances. When enabled, the exported PDF matches the exact Liveboard layout as displayed on screen, rendering visualizations in a continuous layout rather than splitting them across A4 pages.
...
// TODO: [WRITER] Confirm the import merge behavior — does a duplicate check on `obj_id` update the existing view, or skip the import?
