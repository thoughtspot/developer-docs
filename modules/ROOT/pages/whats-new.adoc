...
==== Date format change in the API response [tag redBackground]#Breaking Change#

After your instance is upgraded to 9.8.0.cl and Custom Calendar is enabled on your instance, the data returned in the API response shows the date and time in the  `{"v":{"s":694252800}}` format.

Before Custom Calendar is enabled, the API output shows the date and time data in Epoch time format.
...
After Custom Calendar is enabled, the date and time data is shown in the following format:
----
...
* `v` indicates value
* `s` indicates the start date
* `e` indicates the end date

If the current API implementation in your environment is designed to use the date output from the API in Epoch format, this change may break your current setup.

For more information and assistance, contact ThoughtSpot Support.
