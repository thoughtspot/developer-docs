...
Developers can use these events to perform the following actions:

* Listen for key Spotter user interactions and system events such as downloads, edits, visual changes, and Spotter-specific query responses and emit embed events.
...
For a complete list of events, see xref:api-changelog.adoc[Visual Embed SDK changelog].

=== Primary action configuration for Liveboard visualizations

You can now replace or override the action assigned to the primary button that appears on visualization tiles. The `primaryAction` parameter in the Visual Embed SDK allows you to set that primary action for the Liveboard visualizations.
