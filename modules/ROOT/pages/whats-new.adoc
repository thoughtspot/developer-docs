...
== Upcoming changes (Version 9.2.0.cl and later)

=== Cross-filters and contextual menu changes

Starting with 9.2.0.cl, ThoughtSpot supports brushing and linking of visualizations using cross-filters. Cross-filters allow you to present a coordinated view of a Liveboard by applying filters across all visualizations based on the current selection. As part of this feature, the contextual menu that currently opens on right-clicking a data point on a visualization will appear as a __left-click-triggered__ pop-up menu. This change makes features like *Drill down* more easily discoverable and improves the user experience for those accessing ThoughtSpot on touch devices.

If your app uses `EmbedEvent.VizPointClick` to listen to click events on an embedded Liveboard visualization, once you activate the Cross-filters feature, you will notice that the `VizPointClick` event is triggered whenever a user clicks a contextual menu action, such as *Drill down* or *Filter*.

If you are using `EmbedEvent.VizPointClick` to trigger actions such as updating runtime filters on a Liveboard visualization, the click events from the contextual menu may impact your app’s current workflow. To avoid this issue, use the following settings in the Visual Embed SDK v1.21.0 and later:

Define your preferred settings for the contextual menu::
By default, the contextual menu will open when a user left-clicks on a data point. You can force your app to open the contextual menu as a right-click menu by setting the `ContextMenuTrigger` property to `right-click` in the SDK. This will allow your app to fire the `VizPointClick` event as per your current workflow.

Turn off cross-filters::
You can turn off the cross-filters feature by setting `Action.CrossFilter` and `Action.RemoveCrossFilter` as hidden attributes in the SDK. For more information about hiding a menu action, see xref:embed-actions.adoc[Show or hide menu items].

Use VizPointRightClick to monitor right-click events::
To listen to the right-click events on a chart or table, use the `EmbedEvent.VizPointRightClick` event.
...
The 9.1.0.cl release includes bug fixes and improvements to Search bar embed experience, new Liveboard experience, and REST API v2.0 Playground experience.

=== GraphQL playground [beta betaBackground]^Beta^
The *Develop* tab in the ThoughtSpot UI introduces a GraphQL playground to explore the REST API v2.0 operations and try out GrpahQL queries and mutations. To enable this feature on your instance, contact ThoughtSpot Support.

=== Visual Embed SDK version 1.20.0

For information about the new features and enhancements introduced in Visual Embed SDK version 1.20.0, see xref:api-changelog.adoc[Visual Embed changelog].
...
Starting with 9.0.0.cl, the REST API v2 [beta betaBackground]^Beta^ API endpoints are deprecated and removed from the REST API v2 Playground. A set of new endpoints are now generally available (GA) on all ThoughtSpot instances.
...
In 9.0.0.cl, the *Develop* tab in ThoughtSpot UI introduces a GraphQL playground to explore the REST API v2.0 operations and try out GrpahQL queries and mutations.
////
...
This feature is available only on deployments that have the new Liveboard experience enabled. For more information, see xref:enable-liveboardv2.adoc#_customize_liveboard_tabs[Customize Liveboard tabs].
