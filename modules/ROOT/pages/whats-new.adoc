...
In PDF downloads, Liveboard tabs can now be rendered in a single page matching the UI layout. This feature can be enabled or disabled via `isContinuousLiveboardPDFEnabled` in the SDK. Setting this flag to `false` returns to the paginated PDF view.
...
*Upgrade notes*: ⚠️Variable update and delete API endpoints are deprecated and replaced with new API endpoints. Refer to xref:rest-apiv2-changelog.adoc#version_26_4_0_cl_april_2026[REST API changelog] and xref:deprecated-features.adoc[Deprecation announcements]. +
