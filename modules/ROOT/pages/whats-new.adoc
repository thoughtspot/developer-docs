...
The Theme builder now has an AI mode which enables developers to explore and preview style customization of their embedded application’s branding using natural language instructions and uploaded brand assets, without needing to write or debug CSS manually. You can execute complex updates like applying colors directly from a PDF brand guide, updating all buttons shape with higher contrast, matching a header to a dark background based on a screenshot, or importing typography and spacing from a JSON file. AI Mode interprets your intent and applies the changes instantly.
