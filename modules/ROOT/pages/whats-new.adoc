...
=== Liveboard embed enhancements

You can now create a visual group of Answers and note tiles on an embedded Liveboard and style parts of the Liveboard, groups and Answers using the new styling panel and the following CSS variables in style customization framework of the SDK.
...
For more information, see xref:css-variables-reference.adoc#_grp-style[CSS variables reference].

=== Lazy loading of visualizations on a Liveboard

You can now enable lazy loading of Liveboard visualizations when the `fullHeight` property is enabled. When set to `true`, the visualizations in the embedded Liveboard are loaded incrementally as the user scrolls.
...
You can now set runtime filters and runtime Parameters for Spotter embed.
...
In full aplication embed, developers can now enable the new navigation and modular home page  experience with the sliding left navigation panel. The new experience also organizes the top navigation menu into separate persona-based contextual section. The new navigation experience is disabled by default. To enable this feature on your instance, contact ThoughtSpot Support and then configure the xref:AppViewConfig.adoc#_discoveryexperience[`discovery experience`] property in the SDK.

=== Connection configuration
ThoughtSpot now supports multiple configurations per connection feature for its embedded users. This allows developers to define and manage several distinct connection configurations under a single data connection object in ThoughtSpot. This is currently supported for Snowflake, Databricks, and BigQuery.

For more information, see xref:connection-config.adoc[Connection Configuration].

=== Custom calendars

You can now create and manage custom calendars for a given a connection. For more information, see the API documentation in the REST API v2 Playground.
