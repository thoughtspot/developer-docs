...
ThoughtSpot users will now have the option to switch to a new home page and navigation experience.
The new experience has a redesigned left-hand navigation, to go directly to your Liveboards, Answers, SpotIQ Analysis, and Monitor Subscriptions.
The top navigation from the classic experience is now replaced with an Application switcher image:./images/app_switcher.png[the app switcher menu]. This application switcher gives separate contextual navigation sections based on the requirements for a specific user.

This new home page experience is disabled by default. It can be enabled in the *Profile* settings.

For more information, see xref:newUI.adoc[New home page and navigation].
...
=== Ask Sage [earlyAccess eaBackground]#Early Access#
ThoughtSpot application users can now ask follow-up questions and seek clarifications about visualizations and Answers generated using natural language search. The **Ask Sage** action on Liveboard visualizations and and **Ask a follow-up** action search results page allow you to start a conversation with the AI analyst, ask successive questions, and refine the Answer retrieved from a Natural Language Search query.
