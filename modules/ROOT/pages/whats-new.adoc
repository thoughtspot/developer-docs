...
* Configuration controls in the SDK to disable or hide the data source, and show or hide sample questions.
* Customization controls for CSS overrides to change the appearance of Spotter conversation and prompt panels, and  new action IDs for menu customization on the Spotter page.
* Additional options in the link:https://try-everywhere.thoughtspot.cloud/v2/#/everywhere/playground/spotter[Spotter Playground experience, window=_blank], which allow users to explore the customization settings available in the SDK.
