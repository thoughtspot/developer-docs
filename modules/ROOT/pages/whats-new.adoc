...
The changes introduced for Spotter embedding in SCAL-242427 include a comprehensive set of new embed events and host-triggered actions to enable robust two-way communication between the embedded Spotter component and the host application. These enhancements allow developers to:

Listen for key Spotter user interactions and system events (such as downloads, edits, visual changes, and Spotter-specific query responses) via embed events. This supports analytics, telemetry, and custom integrations.
Trigger specific host events from the application to programmatically control Spotter behavior, such as starting a search, resetting the conversation, saving, downloading, or pinning results.
Supported embed events include: AnswerChartSwitcher, Alert, DownloadPDF, DownloadPNG, DownloadXLS, Pin, Save, Reset, TableVizRendered, Edit, LastPromptDeleted, LastPromptEdited, PreviewData, Error, SpotterQueryTriggered, and SpotterData. All Spotter-related events include message ID, conversation ID, and viz ID where applicable.

Supported host events include: SpotterSearch, Edit, Pin, Save, DownloadPDF, DownloadPNG, DownloadXLS, AnswerChartSwitcher, EditLastPrompt, DeleteLastPrompt, PreviewData, and ResetSearch. SpotterSearch does not require a conversation ID, and ResetSearch resets the entire Spotter conversation state.

These changes apply to Spotter Embed, Spotter Agent Embed, and Full App Embed, ensuring consistent event and action support across all major Spotter embedding scenarios

=== Primary action configuration for Liveboard visualizations

The primaryAction parameter in the Visual Embed SDK specifies the main action button to display at the top of a visualization for Liveboard and App Embed scenarios. It allows you to set which primary action (such as Download) is shown prominently to users. This parameter is optional and is supported in AppEmbed and LiveboardEmbed components. For example, you can set primaryAction: Action.Download to make the Download action the main button on the visualization interface

=== Ability to search metadata by subtypes in REST APIs
The subtypes parameter in POST /api/rest/2.0/metadata/search is intended to further filter metadata objects by their sub-category within a main type. For example, if you are searching for metadata objects of type LOGICAL_TABLE, you could use subtypes to specify whether you want only WORKSHEET, TABLE, or VIEW objects. However, querying metadata objects by subtypes is not supported in the current release of REST API v2.0, so including subtypes in your request will not have any effect at this time.
