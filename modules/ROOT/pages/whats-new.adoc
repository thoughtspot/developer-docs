...
=== Email customization per Org
ThoughtSpot embedded users can now customize their automated email notification for each Org through REST APIs v2. You can now customize features such as the logo, style, sender domain, and email address, as well as white-label or remove ThoughtSpot-specific content from all notification emails, thus ensuring a consistent brand experience.

For more information, see xref:customize-email-apis.adoc<Customize email settings>.

=== Publishing content to Orgs [beta betaBackground]^Beta^

The publishing feature [beta betaBackground]^Beta^ enables administrators to publish objects from the Primary Org to other Orgs on a multi-tenant instance. Unlike the deployment method that relies on TML import, the publishing feature allows propagating objects from the default Org to one or several target Orgs without creating copies of the objects. This feature simplifies the deployment process for ThoughtSpot administrators, especially when the same object needs to be made available to multiple Orgs on an instance.

The publishing feature also provides the ability to customize the object properties of the underlying Table or Connection for each Org using variables. Using ThoughtSpot REST APIs, you can create a variable for each Org and use these variables to parameterize Table properties such as database, schema, and table names, or Connection properties such as account name, role, or warehouse. The published objects reference the variable values substituted for their specific Orgs and seamlessly receive updates from the main object in the Primary Org. This approach enables administrators to easily propagate and deploy content, eliminate the need for duplicate objects and avoid higher memory usage resulting from multiple copies of the same object.
