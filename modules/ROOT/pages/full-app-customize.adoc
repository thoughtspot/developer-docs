...
Before updating the UI experience, review the xref:full-app-customize.adoc[key features and limitations] of each UI experience mode, and verify whether the necessary customization controls for your embedding use case are available in the Visual Embed SDK.
...
_User profile level switch: If enabled, users can override the navigation and home page experience from their profile settings. If not visible, the experience is controlled at the cluster or org level and cannot be changed by the user._
...
[width="100%", cols="5,5,7"]
[options='header']
|===
|If +
`modularHomeExperience` is |And V3 experience is | Then, the resulting UI experience is
...
|Is `true`  | Is not set | V2 navigation and modular home page

|Is `false` / Not set / incorrect | Is not set | V1 classic experience.
