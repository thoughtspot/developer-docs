...
*Action required:* Update your `AppViewConfig` to use the `discoveryExperience` object with `primaryNavbarVersion: PrimaryNavbarVersion.Sliding` and `homePage: HomePage.ModularWithStylingChanges` to ensure your application renders correctly. See <<_upgrade_to_the_v3_experience,Upgrade to the V3 experience>> for step-by-step guidance.

// NOTE TO WRITER: The previous notice read "will be deprecated in an upcoming release in 2026". This has been updated to confirm deprecation is effective 26.8.0.cl. Verify the exact release wording with the product team before publishing.
====
...
// NOTE TO WRITER: The V1 and V2 legacy sections below are retained for reference for customers who have not yet migrated. Do not remove these sections until V1/V2 is fully sunset and migration deadline has passed. Add a sunset date here when confirmed.

==== Upgrade from the V2 experience to V3 experience [.version-badge.deprecated]#Deprecated#

[NOTE]
====
The V2 experience is deprecated as of 26.8.0.cl. The following section is retained as a migration reference. If you are still on V2, follow these steps to upgrade to V3.
====

To upgrade from V2 to V3, update your `discoveryExperience` configuration to include `primaryNavbarVersion: PrimaryNavbarVersion.Sliding` and `homePage: HomePage.ModularWithStylingChanges`.
