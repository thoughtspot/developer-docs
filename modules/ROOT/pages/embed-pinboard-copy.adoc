...
:page-pageid: embed-liveboard-copy
...
To embed a Liveboard, you need the following access and setup:
...
== Create an instance of the LiveboardEmbed object
...
== Customizing embedded Liveboard interface
...
Parameters such as link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_hideliveboardheader[`hideLiveboardHeader`],link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_hidetabpanel[`hideTabPanel`], link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_isliveboardheadersticky[`isLiveboardHeaderSticky`], link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_showliveboardtitle[`showLiveboardTitle`], and link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_showliveboarddescription[`showLiveboardDescription`] control various aspects of the standard embedded Liveboard experience. Note the phrasing of the property name and the description in the documentation to understand whether `true` enables or disables the particular feature.
...
The link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_hideliveboardheader[`hideLiveboardHeader`] property removes the entire header area above the Liveboard, including filters and the overall Liveboard menu, which is a common pattern for "read-only' use cases or rebuilding your own menus and buttons using xref:embed-events.adoc[HostEvents].
...
[NOTE]
====
When link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_fullheight[`fullHeight`] is set to `true`, the SDK ignores the `isLiveboardHeaderSticky:true` setting, and the Liveboard header will not be sticky.
====

=== Customize Liveboard tabs

By default, the first tab created on a Liveboard is set as the home tab. You can set any tab as an active tab using the link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_activetabid[`activeTabId`] property in the Visual Embed SDK as shown in the example here:
...
link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_visiblevizs[`visibleVizs`] and link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_visibletabs[`visibleTabs`] allow for limiting the experience for certain users from a Liveboard with many more elements.
...
When turned on, the link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_enable2columnlayout[`enable2ColumnLayout`] allows for customising the Liveboard breakpoint width for embedded users.
...
[NOTE]
====
This feature is supported only if compact header is enabled on your Liveboard. To enable compact header, use the link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_isliveboardcompactheaderenabled[`isLiveboardCompactHeaderEnabled`] attribute.
====
...
To enable this feature, set link:https://developers.thoughtspot.com/docs/Interface_LiveboardViewConfig#_isliveboardmasterpiecesenabled[`isLiveboardMasterpiecesEnabled`] to `true`.
