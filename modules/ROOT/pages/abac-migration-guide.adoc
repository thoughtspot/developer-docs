...
The legacy JWT ABAC implementation methods will be deprecated and removed from the product in a future version. These legacy methods will not support new enhancements or JWT workflow improvements. Therefore, We recommend migrating your existing deployments to the xref:abac_rls-variables.adoc[ABAC via RLS] method and testing your rollout for production use cases before the legacy methods are fully deprecated and no longer supported by ThoughtSpot.
