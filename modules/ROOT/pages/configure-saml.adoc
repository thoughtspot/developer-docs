...
To link:https://docs.thoughtspot.com/cloud/9.10.0.cl/saml-okta#_enable_saml_authentication[establish a SAML authentication connection using IAMv2, window_=blank], the *SAML2 IDP* tile needs to be selected from the IdP options in the *Admin* panel.
...
For detailed information on enabling SAML authentication on ThoughtSpot using IAMv2, and attributes, follow the steps listed on this page
