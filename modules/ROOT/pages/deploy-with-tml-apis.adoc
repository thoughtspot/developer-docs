...
== Pre-requisites
For TML files to be portable between any Org on any instance, the objects must have *obj_ids* and all Table and Connection objects must have been xref:metadata-parameterization.adoc[parametirized] with the appropriate xref:variables.adoc[variables] where necessary.
