...
If RBAC is enabled on your instance, the granular download privileges replace the single `DATADOWNLOADING` privilege.
[width="100%" cols="2,4,4"]
[options='header']
|===
|Role type|Privilege|Description
|Download visuals |API: `CAN_DOWNLOAD_VISUALS` +
UI: *Can download visuals*  |Allows users to download data in the PDF or PNG file format.
|Data export |API: `CAN_DOWNLOAD_DETAILED_DATA` +
UI: *Can download detailed data* | Allows users to export data in XLSX or CSV file format.
|===
