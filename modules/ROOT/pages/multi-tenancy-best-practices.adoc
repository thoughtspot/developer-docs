...
Whether users can see each other is controlled by group membership and a property called *shareability*. Please see the documentation on xref:access-control-sharing.adoc#_sharing_visibility[access control and sharing] to learn how to configure the shareability properties for users and groups to achieve separation within a single Org.
