...
If the cloud data warehouse you are connecting to is multi-tenanted as described above, then you will only need one set of data objects in ThoughtSpot. 
...
ThoughtSpot controls content access through the concept of xref:access-control-sharing.adoc[sharing]. Content in ThoughtSpot belongs to its creator (owner), and by default they are the only user who knows the content exists. This allows for self-service creation of new searches and Liveboards.
