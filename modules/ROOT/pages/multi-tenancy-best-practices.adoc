...
While you can share individual tables from connections to users, the best practice is to create link:https://docs.thoughtspot.com/software/latest/models[Models, window=_blank] and only share the relevant Models to end users. Any Liveboards and saved Answers shared to users should only connect to Models.
