...
By default, home page is set as the default page when embedded application loads. To change the default settings, you can use either `pageId` or `path` parameter. If both `path` and `pageId` properties are defined in the Visual Embed SDK, the `path` definition takes precedence.

== Set default page using pageId
...
== Set the default page using path attribute
