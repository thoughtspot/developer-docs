...
:page-title: Set default landing page and navigation
...
* `Page.Search` for the ThoughtSpot *Search data* page
* `Page.Answers` for the *Answers* list page
* `Page.Liveboards` for the *Liveboards* list page
* `Page.Data` for the *Data workspace* page
* `Page.SpotIQ` for the *SpotIQ* analyses list page
* `Page.Monitor` for the *Monitor subscriptions* list page
* `Page.Collections` for the *Collections* list page
* `Page.LiveboardSchedules` for the *Liveboard schedules* list page
...
|Home a| `path: "home"` +
`path: "insights/home"`
|Spotter a| `path: "conv-assist"` +
`path: "insights/conv-assist"`
|Search data a| `path: "answer"` +
`path: "insights/answer"`
|Liveboards list page a|
`path: "home/liveboards"` +
`path: "insights/liveboards"`
|Liveboard page a| `path: "pinboard/<Liveboard-GUID>"`
|Answers list page a|
`path: "answers"` +
`path: "home/answers"` +
`path: "insights/answers"`
|Saved Answer page a| `path: "saved-answer/<answer-GUID>"`
|Data a| `path: "data/tables/"`
|Model, tables, views
a|`path: "data/tables/<object-GUID>"`
|SpotIQ analysis list page a| `path: "insights"` +
`path: "insights/spotiq-analysis"`
|SpotIQ analysis page a| `path: "insight/<spotIQ-analysis-GUID>"`
|Monitor subscriptions list page a| Supported settings include:

* `path: "monitor"` +
* `path: "home/monitor-alerts"` +
* `path: "insights/monitor-alerts"`
|Liveboard schedules list page a|Supported settings include:

* `path: "liveboard-schedules"`
* `path: "home/liveboard-schedules"` +

|Collections list page a|Supported settings include:
...
| `path`
|__String__. The page to navigate to. Accepts the same string values as the `pageId` or `path` attributes (for example, `'answers'`, `'pinboard/<GUID>'`, or a `Page` enum value). +
When `noReload` is set to `true`, this parameter also accepts a number: use `1` to go forward or `-1` to go back in history.

| `noReload`
|__Boolean__. If `true`, navigation is triggered through the `HostEvent.Navigate` event without reloading the iframe. +
*Default:* `false`
|====

[NOTE]
====
`navigateToPage()` requires that `render()` has been called on the `AppEmbed` instance before it is invoked. If `render()` has not been called, the method exits silently with a warning in the console.
====
...
When `noReload` is set to `true`, `navigateToPage()` internally triggers xref:HostEvent.adoc#_navigate[HostEvent.Navigate] with the specified `path` as the event payload. You can also trigger this host event directly to achieve the same no-reload navigation behavior:
...
[NOTE]
====
`HostEvent.Navigate` is applicable only to `AppEmbed`. It is not supported for `LiveboardEmbed`, `SearchEmbed`, or `SpotterEmbed`.
====
...
In this example, the **view-report** action on a Liveboard page calls the `navigateToPage` method to open a specific Answer page when the user clicks the **View report** button.
