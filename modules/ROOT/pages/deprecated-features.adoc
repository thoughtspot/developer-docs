...
The `include_cover_page` and `include_filter_page` options for  `POST /api/rest/2.0/report/liveboard` endpoint are now deprecated for PNG downloads.
...
* For embedded users, starting ThoughtSpot 26.8.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error. While these parameters remain functional within your embedded flow, ThoughtSpot no longer provides official support for them.
* For non-embedded users, starting ThoughtSpot 26.4.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error. If you still have to use these options for your ThoughtSpot instance contact ThoughtSpot support to revert to these legacy settings.
* The `include_cover_page` and `include_filter_page` attributes for PNG exports will eventually be removed in a later release.

Recommended action::
* We recommend that you use the following options for PNG exports:
** `image_resolution`
** `image_scale`
** `include_header`
+
