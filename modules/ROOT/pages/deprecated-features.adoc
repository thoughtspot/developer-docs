...
a|xref:deprecated-features.adoc#PNGFlowDeprecation[Select PNG export options] a|ThoughtSpot Cloud 26.4.0.cl and later | Arpil 2026 | TBD
...
[#PNGFlowDeprecation]
== Select options in the PNG export
API calls to the `POST /api/rest/2.0/report/liveboard` endpoint with `include_cover_page` and `include_filter_page` will result in an error. These attributes are now deprecated.

The Liveboard Report API has improved export options which generate high-quality PNGs that closely match the Liveboard experience. It supports `image_resolution` (up to 3840px wide), `image_scale` (zoom), and allows developers to export a specific tab instead of stitching all tabs vertically.

Impact on your instance::
* For non-embedded users, starting ThoughtSpot 26.4.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint with `include_cover_page` and `include_filter_page` will result in an error. If you still have to use these options for your ThoughtSpot instance contact ThoughtSpot support to enable these settings.
* For embedded users, starting ThoughtSpot 26.8.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint with `include_cover_page` and `include_filter_page` will result in an error.
* The `include_cover_page` and `include_filter_page` attributes will eventually be removed in a later release.

Recommended action::
* We recommend that you start using the following options for PNG exports:
** `image_resolution`
** `image_scale`
** `include_header`
...
Embedded users must contact ThoughtSpot support to enable these settings for PNG downloads on their ThoughtSpot instance.
