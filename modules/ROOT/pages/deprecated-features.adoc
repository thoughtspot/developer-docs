...
* For existing integrations, starting with the ThoughtSpot 26.8.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error.
** If you are currently using the legacy PNG export flow with `include_cover_page` and `include_filter_page`, it will continue to work without interruption.
...
* For new integrations, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error. Use the new PNG export options.
...
* PNG download now support exporting only one tab at a time. If the `tab_identifier` is not specified, the first tab will be downloaded.
