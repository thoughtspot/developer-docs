...
API calls with `include_cover_page` and `include_filter_page` to the `POST /api/rest/2.0/report/liveboard` endpoint will result in an error.

The new export options generate high-quality PNGs that closely match the Liveboard experience. It supports custom resolutions (up to 3840px wide), scaling (zoom), and allows developers to export a specific tab instead of stitching all tabs vertically.
