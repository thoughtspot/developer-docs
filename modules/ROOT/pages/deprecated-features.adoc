...
The `PREVIEW_THOUGHTSPOT_SAGE` privilege is renamed to `CAN_USE_SPOTTER` with the ThoughtSpot 26.3.0.cl release version. The API calls will continue to support both these privileges until ThoughtSpot 26.5.0.cl release version, after which the `PREVIEW_THOUGHTSPOT_SAGE` privilege will be removed.
For ThoughtSpot instances which have enabled RBAC before the 26.3.0.cl release, there will be no automatic changes to the roles created using the `PREVIEW_THOUGHTSPOT_SAGE` privilege. However, the underlying privilege associated with such roles will be appropriately renamed to `CAN_USE_SPOTTER`. The admins for these clusters will be notified to rename the role in accordance with the newer privilege name.
