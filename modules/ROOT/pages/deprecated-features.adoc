...
Starting with 10.4.0.cl, Worksheets are deprecated and disabled by default in ThoughtSpot. In ThoughtSpot Cloud  10.12.0.cl, the ability to create new Worksheets is removed, and all existing Worksheets will be automatically migrated to Models.

Impact on your instance::

All existing Worksheets on your instance will be migrated automatically to Models, and Worksheets will no longer be available after the 10.12.0.cl release.
Any TML import of a Worksheet or adding a dbt connection will result in the creation of a Model instead.

Recommended action::
* If your instance has Worksheets, verify whether they are replaced with Models once your instance is upgraded to the 10.12.0.cl release version.
* If your implementation relies on Worksheets, update your application workflows and integrations to use Models.
