...
API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error. These attributes are now deprecated for PNG downloads.
...
* For embedded users, starting ThoughtSpot 26.8.0.cl release, API calls to the `POST /api/rest/2.0/report/liveboard` endpoint for PNG exports with `include_cover_page` and `include_filter_page` will result in an error.
