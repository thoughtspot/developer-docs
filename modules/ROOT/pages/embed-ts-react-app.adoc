...
To embed a ThoughtSpot saved answer in your React app, use the `SearchEmbed` component with the `savedAnswerGuid` property.
...
const SavedAnswer = () => {
  ...
  return (
    <SearchEmbed
      ...
      savedAnswerGuid="<YOUR_SAVED_ANSWER_GUID>"
      hideSearchBar={true}
      onInit={onInit}
      onLoad={onLoad}
    />
