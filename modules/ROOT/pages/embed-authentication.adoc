...
////
++++
<img src="../../../static/doc-images/images/auth-type-embed.svg" alt="Embed authentication types" usemap="#image-map">
...
== User accounts
Many ThoughtSpot features are tied to individual user accounts with a valid e-mail address. xref:just-in-time-provisioning.adoc[Just-In-Time Provisioning] and user management REST APIs make it easy to create and update user accounts as part of the SSO process. 

For some situations, shared *customer-level* or *role-level* accounts may be more appropriate than *individual* accounts. Please discuss with your ThoughtSpot team which provisioning pattern makes the most sense for your application, and the implications of shared accounts on aspects of ThoughtSpot such as access control and row-level security.
