...
[NOTE]
====
The `run-ts-workflow` skill requires a valid authenticated session with your ThoughtSpot instance. This skill is only available through the authenticated `/mcp` endpoint. It is not available through the unauthenticated `/mcp/docs` endpoint.
====
