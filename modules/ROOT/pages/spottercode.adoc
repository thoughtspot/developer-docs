...
Because code executes as the authenticated user, write operations, such as create, update, delete, and share, generate the same audit log entries as equivalent REST API calls made directly by that user. For details on the events captured and how to retrieve them, see xref:audit-logs.adoc[Audit logs] and xref:logs-api.adoc[Audit logs API].
