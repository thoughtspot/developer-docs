...
When the Host Events v2 framework is enabled, the embedded ThoughtSpot app emits the `subscribed` signal whenever it registers an internal handler for a Host Event.
...
* Triggering events before the embed UI has fully loaded and event handler is subscribed.
