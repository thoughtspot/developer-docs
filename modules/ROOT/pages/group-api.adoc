...
To add a role to a group object, send a `POST` request to the `/tspublic/v1/group/addrole` API endpoint.

[NOTE]
====
The API endpoints for Role administration and assignment are available only if the Role-based access control feature is enabled on your instance. The RBAC feature is in beta and turned off by default. To enable RBAC on your instance, contact ThoughtSpot Support.
====
...
If the Role assignment is successful, the API returns the group metadata with the Role assignment details.
...
[#removeRole]
== Remove a Role assigned to a group
