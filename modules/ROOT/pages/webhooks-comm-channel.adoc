...
To ensure that a communication channel configuration is properly configured and can receive events, use the `/api/rest/2.0/system/communication-channels/validate` API endpoint to validate the communication channel configuration.
...
|`channel_identifiers`|__Array of strings__. IDs or names of the communication channel. Required if no job ID is specified.
...
|`end_epoch_time_in_millis` |__Decimal__. Specify the end time in epoch milliseconds to fetch records created before the specified timestamp.
|=====
