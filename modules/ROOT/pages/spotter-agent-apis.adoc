...
__Available on ThoughtSpot Cloud instances from 10.13.0.cl onwards__.
    ...
    xref:spotter-agent-apis.adoc#_share_a_conversation[Shares a saved Spotter conversation] with one or more users or groups. Use `grant` and `revoke` arrays to manage access. Shared conversations are `READ_ONLY`. +
    ...
    xref:spotter-agent-apis.adoc#_get_share_information[Returns sharing metadata] for a Spotter conversation — the list of principals it is shared with and whether the shared content is outdated. +
    __Available on ThoughtSpot Cloud instances from 26.9.0.cl onwards.__
...
----
For the complete response in one payload, use the xref:spotter-agent-apis.adoc#_send_queries_to_a_conversation_session[`/send` endpoint] instead.
...
* For information about MCP tools, see xref:mcp-integration.adoc[MCP server integration].
    ...
    |=====
    a| `POST /api/rest/2.0/ai/agent/conversations/{conversation_identifier}/share` [.version-badge.new]#New# +
    xref:spotter-agent-apis.adoc#_share_a_conversation[Shares a saved Spotter conversation] with one or more users or groups. +
    __Available on ThoughtSpot Cloud instances from 26.9.0.cl onwards.__
