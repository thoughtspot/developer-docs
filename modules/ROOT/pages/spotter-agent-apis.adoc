...
__Available on ThoughtSpot Cloud instances from 10.13.0.cl onwards. Includes breaking changes in 26.5.0.cl__.
...
| `POST /api/rest/2.0/ai/relevant-questions/` [beta betaBackground]^Beta^ +
...
The `/api/rest/2.0/ai/agent/conversation/create` API endpoint allows creating a new conversation session with Spotter Agent for specific or multi-data context and returns a conversation ID in response.
