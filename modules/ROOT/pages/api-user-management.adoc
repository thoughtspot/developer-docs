...
With link:https://docs.thoughtspot.com/cloud/latest/okta-iam[Identity and Access Management v2 (IAMv2)], users need to provide an email attribute for all users during user creation. Users will also see `SUSPENDED` as a valid user state in the API response for these APIs:
