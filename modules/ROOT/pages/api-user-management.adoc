...
ThoughtSpot introduces a new user state,*SUSPENDED*, for embedded users on IAMv2. With this enhancement, these users do not require a password reset when transitioning between states such as ACTIVE, EXPIRED, or INACTIVE. It also enables seamless migration to IAMv2 for embedded users.

With IAM v2, the users need to give an email attribute for successful user creation. In case no email is provided at the time of user creation, the admin email will be populated for that user.
...
* While creating or updating a user to the EXPIRED or INACTIVE state using using REST APIs, the user will be suspended instead. This change will effectively block user logins without altering existing behavior.

==== Affected APIs
* You can create a user in the SUSPENDED state using the create user API v1 or v2.
* You can change the state of an ACTIVE or PENDING user to SUSPENDED using the update user API v1 or v2.
* To reactivate a SUSPENDED user, simply transition them back to the ACTIVE or PENDING state using the update user API v1 or v2.
...
With the introduction of the SUSPENDED user state, a new UNSUSPEND user button on the user dashboard of the admin page. This button is available for all users in the SUSPENDED state.
Upon clicking this button, the user’s state will transition from SUSPENDED to either ACTIVE or PENDING.
