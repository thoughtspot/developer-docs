...
With link:https://docs.thoughtspot.com/cloud/latest/okta-iam[Identity and Access Management v2 (IAMv2)], users must provide an email attribute for all users during user creation. The email field is mandatory in ThoughtSpot Okta. ThoughtSpot recommends that users enter a valid email address during the user creation process. Administrators can manage user account status for all users. For IAMv2 users, they will also see `SUSPENDED` as a valid user state in the API response for these APIs:
