...
* PENDING
+
This is for IAMv2 users who have been sent a ThoughtSpot account activation email but have not yet logged in.
* LOCKED
+
...
ThoughtSpot introduces a new SUSPENDED user state for embedded users on IAMv2. With this addition, the users do not require a password reset when transitioning between states such as ACTIVE, EXPIRED, or INACTIVE. It also supports seamless migration from IAMv1 to IAMv2 for embedded users.
...
+
`POST /tspublic/v1/user` xref:user-api.adoc#create-user[(Rest API v1)]
+
`POST /api/rest/2.0/users/create` xref:rest-api-v2-reference.adoc#_users[(Rest API v2)]
...
+
`PUT /tspublic/v1/user/{userid}` xref:user-api.adoc#update-user[(Rest API v1)]
+
`POST /api/rest/2.0/users/{user_identifier}/update` xref:rest-api-v2-reference.adoc#_users[(Rest API v2)]
