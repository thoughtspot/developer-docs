...
[NOTE]
On ThoughtSpot instances with 9.8.0.cl, the `name` string supports only [a-z], [A-Z], [0-9], and [-]. Space and other special characters are not supported.
...
|`groupids`|FormData|__String__. The GUIDs of the groups in the Org specified in the API request.
|`orgScope`|Query|__String__. The Org scope. Specify `ALL` to set the Org scope to all Orgs.
|===
