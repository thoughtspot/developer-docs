...
Formula variables are a type of template variable in ThoughtSpot that enable dynamic, context-aware logic in formulas, especially for Row Level Security (RLS) rules. Formula variables can be configured at the Org, user, and data model level to implement clear rule definitions, dynamic assignment at runtime, a more efficient SQL query generation, and separate data access attributes at the organization level.

== Overview
Formula variables are available on ThoughtSpot application instances with 10.15.0.cl or later versions and can be configured by the ThoughtSpot administrators using Variable REST APIs.

You can use these variables in the following scenarios:

* In the xref:rls-rules.adoc[RLS rules] to dynamically restrict access to data. For example, to filter data by region, department, or entitlement, with the value determined by the Org or user context.
* During the Attribute-Based Access Control (ABAC) implementation to enforce user-specific or Org-specific access policies and security filters via JWT tokens.
* For multi-tenant or personalized analytics with user-specific filtering and calculations without duplicating objects or hardcoding values.
...
[IMPORTANT]
====
* Currently, the formula variables can added and managed only via REST APIs.
...
After creating a request, you can assign values to variables and set the scope using the xref:variables.adoc#_define_values_and_scope_for_variables[variable update] API request:
