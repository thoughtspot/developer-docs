...
ThoughtSpot introduces the following API endpoints to programmatically create and manage connections:
...
// Shashi - https://thoughtspot.atlassian.net/browse/SCAL-258315

=== Performance optimization for pre-rendered Liveboards in embed mode
Pre-rendered Liveboards in embed mode automatically update when the application state changes after the shell is pre-rendered. This improves reliability and reduces load times by initializing the application shell in the background, so Liveboards load almost instantly when accessed. These enhancements provide a smoother, more responsive experience, especially for frequently accessed or large Liveboards.
+
...
// Shashi - https://thoughtspot.atlassian.net/browse/SCAL-282526[https://thoughtspot.atlassian.net/browse/SCAL-282526]
[#locale-config-validation]
Accept-Language and preferred locale validation and synchronization::
...
For more information about locale customization for ThoughtSpot Embedded application users, see the https://developers.thoughtspot.com/docs/set-locale[Developer Documentation, window=_blank].




// Shashi - https://thoughtspot.atlassian.net/browse/SCAL-268031[https://thoughtspot.atlassian.net/browse/SCAL-268031] +
[#variable-apis]
Variable API enhancements::


* The `/api/rest/2.0/template/variables/update` endpoint is deprecated. To update the name of a variable, use the `/api/rest/2.0/template/variables/{identifier}/update` endpoint.
* The `/api/rest/2.0/template/variables/update-values` endpoint allows you to add, modify, remove, and replace the values assigned to a variable.
* The `EDITABLE_METADATA_AND_VALUE` attribute for filtering API responses from the +/api/rest/2.0/template/variables/+ search endpoint has been removed.
+
For more information, see https://developers.thoughtspot.com/docs/variables[Variable API documentation, window=_blank].
...
//Shashi - (https://thoughtspot.atlassian.net/browse/SCAL-242427[https://thoughtspot.atlassian.net/browse/SCAL-242427] and https://thoughtspot.atlassian.net/browse/SCAL-271228[https://thoughtspot.atlassian.net/browse/SCAL-271228])
[#spotter-embed-events]
Events framework for Spotter embedding::
Spotter embed now supports a comprehensive set of embed and host events that enable interaction between the embedded Spotter component and the host application. Developers can use these events to:


* Listen to key Spotter user interactions and system events, such as downloads, edits, visual changes, and Spotter-specific query responses, by registering handlers for embed events.
...
* https://developers.thoughtspot.com/docs/embed-sdk-changelog#_version_1_41_0_september_2025[Visual Embed SDK changelog, window=_blank]
* https://developers.thoughtspot.com/docs/Enumeration_EmbedEventhttps://developers.thoughtspot.com/docs/Enumeration_EmbedEvent[EmbedEvent, window=_blank]
* https://developers.thoughtspot.com/docs/Enumeration_HostEventhttps://developers.thoughtspot.com/docs/Enumeration_HostEvent[HostEvent, window=_blank]
...
You can now customize a specific occurrence of a text string in the ThoughtSpot UI using string IDs. String IDs allow you to selectively and precisely override the visible text strings in the UI and prevent unintended text changes. For more information, see xref:customize-text-strings.adoc[Customize text strings].
...
The compact Liveboard header is disabled by default. To enable this feature, set `isLiveboardCompactHeaderEnabled` to `true` in the SDK.
...
For more information, see link:https://docs.thoughtspot.com/cloud/latest/liveboard-filters#_apply_filters_to_specific_visualizations_or_tabs[Apply filters to specific visualizations or tabs].

==== Breakpoint width settings for Liveboards

You can now customize the width of the Liveboard breakpoint on an embedded instance. To enable it, you must set the `enable2ColumnLayout` property to `true`. For more information, see xref:embed-pinboard.adoc#_redefine_liveboard_breakpoint_widths[Redefine Liveboard breakpoint widths].
...
=== Data model and workspace enhancements

==== Column name localization
ThoughtSpot now supports column name and description aliases [beta betaBackground]^Beta^ in a Worksheet or Model, which can be used for localizing the Search and Answer interfaces. This feature is disabled by default on ThoughtSpot instances. To enable this feature, contact ThoughtSpot support.

==== Models

Models are now available on ThoughtSpot embedded instances. You can use Models as data source objects for Answers and Liveboards instead of Worksheets.

Worksheets are deprecated in 10.10.0.sw and will be removed from ThoughtSpot in an upcoming release. If you are embedding the full ThoughtSpot application or the Data Workspace page in your app, you will notice that the Worksheet creation option is disabled by default. However, existing Worksheets continue to be available and editable. To enable Worksheet creation on your instance, contact ThoughtSpot Support.

You can convert a Worksheet to a Model via a REST API POST call to the
...
Several changes have been introduced to improve the ABAC configuration workflows:

* The `user_parameters` property in `auth/token/full` and `auth/token/object` APIs that was used for the beta implementation of ABAC is now deprecated.
* Starting from 10.10.0.sw, administrators are advised to xref:abac-user-parameters.adoc[use the `/api/rest/2.0/auth/token/custom` API endpoint] to define security attributes for ABAC implementation.

If you have implemented ABAC using `auth/token/full` or `auth/token/object` API endpoints, you can migrate your implementation to start using the `/api/rest/2.0/auth/token/custom` API endpoint. For more information, refer to xref:abac-user-parameters.adoc[ABAC documentation].

=== Orgs context for custom links
Developers embedding ThoughtSpot in their application can now include the Org context in ThoughtSpot URLs using custom link settings.
...
=== Theme builder for ThoughtSpot interface customization
...
Developers embedding ThoughtSpot can preview customization options in Theme Builder for various embed components, including Liveboard, Visualizations, Search page, and the full application experience. Additionally, you can view and download the default CSS variables in JSON format and upload a JSON file containing custom values for these CSS variables. For more information, see xref:theme-builder.adoc[Theme Builder].
...
For more information, see link:https://developers.thoughtspot.com/docs/mobile-embed[Mobile embed documentation].

=== Selective user access

With selective user access, you can allow users without administrator or developer privilege to securely access non-embedded ThoughtSpot application pages.

For more information, see xref:selective-user-access.adoc[User access to non-embedded content].

=== Visual Embed SDK

ThoughtSpot recommends using Visual Embed SDK version 1.39.x or later. For information about the new features and enhancements introduced in these versions, see link:https://developers.thoughtspot.com/docs/embed-sdk-changelog[Visual Embed changelog, window=_blank].
...
==== Search Assist
The Search Assist feature for Worksheets and the **Enable Search Assist** checkbox in Search Embed Playground are removed from ThoughtSpot in 10.10.0.sw. If your embedding application uses the `enableSearchAssist` property, you may want to update your deployment.

For more information, see link:https://developers.thoughtspot.com/docs/deprecated-features#_search_assist[Deprecated features^].

==== Application background customization

In the 10.10.0.sw release, the application background customization option on the **Admin** > **Style customization** and **Develop** > **Customizations** > **Styles** is deprecated. For more information, see link:https://developers.thoughtspot.com/docs/deprecated-features#_application_background_customization_via_ui[Application background customization^].
