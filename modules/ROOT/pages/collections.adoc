...
* Collections can be created by any user, and needs no special user privileges for the Org.
...
|`name_pattern` a|__String__. Required. Specify any case-insensitive pattern to match a Collection, or name of a Collection. Use `%` to perform a wildcard search by name.
...
|`collection_identifiers` a|__Array__. Optional. GUID of the Collection to be searched.
...
|`sort_options` a|__Array__. Optional. To sort the results specify the field to apply the sort on, and the sort order.
|=====
