...
To migrate the JWT ABAC implementation with filter rules to the ABAC via RLS method, complete the following checks:
...
[NOTE]
====
To update variable values for a user, use the `/api/rest/2.0/auth/token/custom` or `POST /api/rest/2.0/template/variables/{identifier}/update-values` API endpoint.
====
