...
With Ask Sage [earlyAccess eaBackground]#Early Access#, embedded application users can ask follow-up questions on a visualization generated from a Natural Language Search query, converse with AI analyst, and refine results. To enable this feature, set `enableAskSage` to `true`.
...
For more information, see xref:AppViewConfig.adoc[AppViewConfig].
