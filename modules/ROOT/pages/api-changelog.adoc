...
Developers can now use string IDs to override a specific occurrence of a text string in the ThoughtSpot UI. The `stringIDs` object in the customization property allows passing IDs of a specific occurrence of a text string and override the visible text in the UI.

To view the string IDs, use the `exposeTranslationIds` boolean flag and use these IDs in the customization code to replace a specific string.

You can load the string ID overrides from a URL via `StringIDsUrl` attribute.
...
The deprecated components with old names will continue to work until further notice.
