...
[discrete]
===== Starter prompt configuration for Spotter embed
The SDK introduces starter prompt configuration properties for the embedded Spotter interface. The `StarterPromptsConfig` interface allows you to set or disable specific elements such as quick search, deep analysis, and data literacy pills on the Spotter page.  \

The `starterPrompts` object allows you to enable
...
[discrete]
===== Spotter starter-prompt pill actions
The SDK introduces the following `Action` IDs to control the visibility of starter prompt pills in the Spotter chat interface.  Spotter chat starter-prompt pills. You can use these action IDs to show or hide specific starter-prompt categories in the embedded Spotter interface.


The Basic Search starter prompt pill.

Starter prompt pills are the row of category buttons shown above the Spotter chat input before a conversation starts. They give a new user ready-made questions to click instead of typing one from scratch, and disappear once the conversation begins. There are three, one per action below.

Clicking this pill opens a card of suggested simple questions; picking one submits it to Spotter right away. Visible by default, and hidden along with the other pills when the surface itself is off

* `Action.QuickSearchPill` — Controls the *Basic Search* starter-prompt pill shown above the Spotter chat input.
* `Action.DeepAnalysisPill` — Controls the *Deep Analysis* starter-prompt pill.
* `Action.DataLiteracyPill` — Controls the *Data Literacy* starter-prompt pill. This pill submits a fixed prompt describing the connected data source.

[source,JavaScript]
----
const spotterEmbed = new SpotterEmbed('#ts-embed', {
  // Hide the Deep Analysis pill
  disabledActions: [Action.DeepAnalysisPill],
  // Alternatively, hide all three:
  // disabledActions: [Action.QuickSearchPill, Action.DeepAnalysisPill, Action.DataLiteracyPill],
});

spotterEmbed.render();
----

These action IDs can be passed in `disabledActions`, `hiddenActions`, or `visibleActions` on any embed type where Spotter is available (`SpotterEmbed`, `AppEmbed`, `LiveboardEmbed`).

For more information, see xref:embed-actions.adoc[Show or hide actions] and xref:customize-spotter-embed.adoc#_spotter_starter_prompts[Spotter quick starter prompts].
...
The SDK introduces the `PreRenderConfig` interface, which bundles pre-rendering settings into a single object:

* `preRenderId` — Unique identifier for the pre-rendered embed instance.
* `preRenderContainer` — DOM element or selector for the pre-render container.
* `doNotTrackPreRenderSize` — When `true`, disables automatic size tracking of the pre-render container.
* `zIndex` — Z-index for the pre-rendered iframe.

Use the new `preRenderConfig` property on `BaseViewConfig` instead of the individual top-level properties.
...
const embed = new LiveboardEmbed('#embed-container', {
  ...
  preRenderConfig: {
    preRenderId: 'my-pre-render',
    preRenderContainer: document.getElementById('pre-render-div'),
...
[IMPORTANT]
====
The following top-level properties are now [.version-badge.deprecated]#Deprecated# in favor of `preRenderConfig`:

* `preRenderId`
* `preRenderContainer`
* `doNotTrackPreRenderSize`

The deprecated properties continue to work but will be removed in a future release. Migrate to `preRenderConfig` when updating your integration.
====
...
// Strongly typed trigger call
const response: TriggerResponse = await embed.trigger(
  HostEvent.UpdateRuntimeFilters,
  { filters: myFilters } as HostEventRequest,
);
----
...
const embed = new LiveboardEmbed('#embed-container', {
  ...
  customizations: {
    style: {
      customCSS: {
        variables: {
          '--ts-var-unsaved-filter-indicator-color': '#FF6B35',
        },
      },
    },
  },
...
const embed = new AppEmbed('#embed-container', {
  pageId: Page.LiveboardSchedules,
});
...
* `--ts-var-liveboard-insight-tile-background` — Background color of the insight tile.
* `--ts-var-liveboard-insight-tile-icon-color` — Color of the insight tile icon.
* `--ts-var-liveboard-insight-tile-success-color` — Color used for positive/success indicators.
* `--ts-var-liveboard-insight-tile-failure-color` — Color used for negative/failure indicators.
...
[discrete]
===== Lazy loading defaults changed for full-height embeds

// SOURCE: SCAL-333385

When `fullHeight` is set to `true` on `AppEmbed` or `LiveboardEmbed`, the following lazy loading parameters now **default to enabled** (previously they defaulted to disabled):

* `lazyLoadingForFullHeight` — defaults to `true`
* `enableScrollableContainerLazyLoading` — defaults to `true`
* `lazyLoadingMargin` — defaults to `'500px 0px'`

This improves initial load performance for large Liveboards. If your integration relies on all visualizations loading immediately when `fullHeight` is enabled, you must now explicitly set these parameters to `false`.

[source,JavaScript]
----
// To restore previous behavior (all visualizations load immediately):
const embed = new LiveboardEmbed('#embed-container', {
  liveboardId: '<liveboard-guid>',
  fullHeight: true,
...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#  a| The `AddRemoveColumns` event is now available in the SDK. For more information, see xref:event-embedEvents.adoc[Events reference].
|====
...
|[tag greenBackground]#NEW FEATURE#  a| The SDK now supports the `UpdateRuntimeFilters` host event. For more information, see xref:events-hostEvents.adoc[Events reference].
|====
