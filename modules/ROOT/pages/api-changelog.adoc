...
The V3 navigation and home page experience is now the default for all embedded ThoughtSpot applications. The classic (V1) and V2 experience modes are deprecated. SDK flags targeting V1 or V2 variants are silently ignored from this release.
...
The following features are now enabled by default in SDK v1.51.0. Explicitly setting any of these to `false` in your configuration continues to be honored:

// TODO: verify with engineering — confirm exact SDK property names in AppViewConfig / LiveboardViewConfig for each flag as shipped in v1.51.0
// SOURCE: SCAL-295712
...
Embedded applications can now configure starter prompts for first-time Spotter users using the `starterPrompts` object in `SpotterViewConfig`. This feature is disabled by default for embedded applications (`enabled: false`).

`enabled`:: Set to `true` to display starter prompts when users open Spotter for the first time.
`categories`:: Array of prompt category objects. Each object accepts `key`, `label`, and `questions` (array of strings).

// TODO: verify with engineering — confirm the complete starterPrompts schema and all supported keys in SDK v1.51.0
// SOURCE: SCAL-293483
...
* `EmbedEvent.Error` +
The `EmbedEvent.Error` now also emits when a HostEvent dispatch fails due to missing or inactive listeners.
