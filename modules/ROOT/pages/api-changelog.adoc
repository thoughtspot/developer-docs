...
== Version 1.28.0, Coming soon

[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE# a| The SDK provides `AnswerService` class to trigger answer service with the custom action payload.
You can use this service to run GraphQL queries in the context of the Answer with a custom action trigger. For more information, see link:https://developers.thoughtspot.com/docs/Class_AnswerService[AnswerService].
|[tag greenBackground]#NEW FEATURE#  a| The following `HostEvent` and `Action` enumeration members are available with the SDK:

** `HostEvent.GetAnswerSession` +
Gets Answer session data for Search Answer or Liveboard Visualization in the embedded view.
** `Action.VerifiedLiveboard` +
Shows or hides the *Verified Liveboard* banner.
|====
