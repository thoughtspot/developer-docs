...
If Spotter is enabled on your instance, the *Spotter* button appears by default as the primary action on embedded Liveboard charts; if Spotter is not enabled, the *Explore* button is shown. If you want to replace primary action with a different action, use the `primaryAction` attribute.

For more information, see xref:embed-actions.adoc#primaryAction[Override default primary action].
