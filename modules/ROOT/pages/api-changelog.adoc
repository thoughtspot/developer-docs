...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE# a| Host event parameters
The following HostEvents now allow custom parameters to programmatically set object properties:
...
* `HostEvent.Pin`
Allows adding custom properties for visualization ID, name, and description, Liveboard ID, and Tab ID. When these parameters are defined, the event triggers an action to save the Answer in the Liveboard specified in the code, without opening the *Pin* modal.
|[tag greenBackground]#NEW FEATURE# a| Configuration and customization controls for Spotter embed
...
The *Preview data* button on Spotter conversation panel.
...
|[tag greenBackground]#NEW FEATURE# a| Configuration controls for Liveboard embed
