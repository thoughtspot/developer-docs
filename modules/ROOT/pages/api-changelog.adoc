...
== Version 1.38.0, May 2025

[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#  a| *String IDs for text customization* +
Developers can now use string IDs to override specific occurrences of a text string in the ThoughtSpot UI. The `stringIDs` object in the customization interface allows passing the IDs of a specific text string and override the visible text in the UI.

To help you locate the string IDs, SDK provides the `exposeTranslationIds` attribute. By setting `exposeTranslationIds` to `true` in the Playground, you can view and copy the string IDs of UI text.

Additionally, the SDK also supports the `StringIDsUrl` property, which allows loading a JSON map with string IDs and text overrides from a hosting URL and apply text changes.
