...
|[tag greenBackground]#NEW FEATURE# |You can now use the `HostEvent.GetIframeUrl` to get the iframe src URL from the Visual Embed Playground. Use this event to generate the iframe URL when embedding ThoughtSpot in apps Salesforce or Sharepoint without using the Visual Embed SDK.
