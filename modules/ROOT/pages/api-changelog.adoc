...
[discrete]
===== Spotter embedding
The SDK introduces starter prompt configuration properties for the embedded Spotter interface. The `StarterPromptsConfig` interface allows you to enable and configure properties of elements such as quick search, deep analysis, and data literacy pills on the Spotter page.

The SDK also provides the following action IDs for disabling or hiding these elements:

* `Action.QuickSearchPill` +
Controls the *Basic Search* starter-prompt pill shown above the Spotter chat input.
* `Action.DeepAnalysisPill` +
Controls the *Deep Analysis* starter-prompt pill.
* `Action.DataLiteracyPill` +
Controls the *Data Literacy* starter-prompt pill. This pill submits a fixed prompt describing the connected data source.

|[tag greenBackground]#NEW# a|

[discrete]
===== Spotter starter-prompt customization
The `starterPrompts` object in the `spotterChatConfig` interaface allows you to customize the labels and questions in the **Quick search** section.

For more information, see xref:customize-spotter-embed.adoc#_spotter_starter_prompts[Spotter quick starter prompts].
