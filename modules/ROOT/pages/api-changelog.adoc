...
The SDK now includes the `hideObjectSearch` property to allow developers to hide the object search button on the navigation bar in their full application embed.

|[tag greenBackground]#NEW FEATURE#  a| *Events* +

In this version, the SDK includes the following event handlers:
...
The SDK introduces `lazyLoadingForFullHeight` parameter to load the visualizations progressively on your embedded Liveboard.
This parameter works in conjunction with the `fullHeight` attribute. When both these attributes are enabled, the visualizations in the current viewport are loaded initially and the additional visualizations on the Liveboard page are  loaded as the user scrolls.

Note that your ThoughtSpot instance requires 10.12.0.cl or later versions to use these attributes effectively in your embedded apps.
|====
