...
Embedded Spotter interface now supports conversation sharing. Users can share a saved Spotter conversation with other users and groups directly from the embedded interface, with view access.

To enable and manage the visibility of this feature and its workflows, the SDK introduces the following proerties:

* `Action.ShareSpotterConversation`: Action ID to control the visibility of the *Share* option in embedded Spotter conversations. Use it in `visibleActions`, `hiddenActions`, or `disabledActions`.
* `disableSpotterConversationSharing`: Turns off conversation sharing in the embedded view. Default: `false`.
* `HostEvent.ShareSpotterConversation`: Triggers the share dialog programmatically from the host application.
* `EmbedEvent.SpotterConversationShared`: Emitted when a user shares a conversation.

For more information, see  share conversations programmatically via REST API, see xref:spotter-agent-apis.adoc#_sharing_spotter_conversations[Sharing Spotter conversations].

|[tag greenBackground]#MODIFIED# a|

[discrete]
===== Full height setting with lazy loading defaults for Liveboards
...
[discrete]
===== Liveboard Schedules page in full application embedding
The `Page.LiveboardSchedules` enum value (`'liveboard-schedules'`) is now available for full application embedding. Use this value to navigate users directly to the Liveboard schedules list page.
