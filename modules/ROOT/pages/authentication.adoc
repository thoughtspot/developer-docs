...
To get an access token that grants full access to ThoughtSpot, send a `POST` request to the `/api/rest/2.0/auth/token/full` endpoint with `username`, `password`, and other attributes in the request body:
...
To get a token that grants read-only access to a ThoughtSpot metadata object, send a `POST` request to the `/api/rest/2.0/auth/token/object` endpoint with `username`, `password`, and `object_id` and other attributes in the request body:
...
To get an access token that grants full access to ThoughtSpot, send a `POST` request body with `username` and `secret_key` along with other attributes to the  `/api/rest/2.0/auth/token/full` endpoint:
...
To get a token that grants a `READ-ONLY` access to a specific metadata object, send a `POST` request body with `username`, `secret_key`, `object_id` and other attributes to the `/api/rest/2.0/auth/token/object` API endpoint:
