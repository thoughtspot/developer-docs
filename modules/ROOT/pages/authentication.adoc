...
|Basic authentication/Cookie-based authentication| In this method, a REST client sends an API request to the xref:authentication.adoc#loginTS[api/rest/2.0/auth/session/login] endpoint with the `username` and `password`. A successful login returns a session cookie from ThoughtSpot, which can be included in subsequent REST API calls.
...
|xref:authentication.adoc#_generate_a_custom_token_for_abac_implementation[Custom access token] |**Use case**: Assigns variable values to users when authenticating them, enabling ABAC via RLS rules when logging users in ThoughtSpot. +
...
* ThoughtSpot recommends using the `Full Access Token` for users, unless there is a need to set up ABAC via RLS, in which case, a custom token is required.
...
The token issued from ThoughtSpot can be used to log in a user. Note that the token is necessary only during the login process, after which any request to ThoughtSpot will include session cookies identifying the signed-in user.
...
|=====
=======
If you are generating an authentication token for a new user in ThoughtSpot, you can use the `auto_create` option to provision a user and obtain a token for that user. Creating a new user requires request attributes such as username, email address, display name, groups, and Org details.
>>>>>>> 527f505a (JWT doc updates)
...
=======
The following sample shows the request format to provision a new user just-in-time via `/api/rest/2.0/auth/token/full` API endpoint and generate a full access token:

If the username already exists in ThoughtSpot, setting `auto_create` to `true` does not create a new user and updates user properties such as display name, email, and group entitlements.
>>>>>>> 527f505a (JWT doc updates)
...
==== Example request
=======
The following sample shows the request format to provision a new user just-in-time via `/api/rest/2.0/auth/token/custom` API endpoint and generate a token with ABAC via RLS variable values.
