...
When `auto_create` is set to `true` in the API request, these endpoints allow you to create a user at the time of authentication if the user does not already exist. When provisioning a new user, ensure that you include `display_name`, `email`, and group details in the API.
...
If the username already exists in ThoughtSpot, setting `auto_create` to `true` creates a new user and updates user properties such as display name, email, and group entitlements.
...
The following sample shows the request format to provision a new user just-in-time via `/api/rest/2.0/auth/token/object` API endpoint and get an authentication token that grants access to a specific metadata object in ThoughtSpot:
...
The following sample shows the request format to provision a new user just-in-time via `/api/rest/2.0/auth/token/custom` API endpoint and generate a token that provides access to a specific metadata object.

If the username already exists in ThoughtSpot, setting `auto_create` to `true` creates a new user. However, user properties such as display name, email, Org and group entitlements will not be updated.
...
[NOTE]
====
If `auto_create` is set to `true` and the username specified in the API request already exists in ThoughtSpot, the `/api/rest/2.0/auth/token/full` and `POST /api/rest/2.0/auth/token/object` APIs , the user properties such as the display name, email, Org and group assignment will not be updated with new values.
====
