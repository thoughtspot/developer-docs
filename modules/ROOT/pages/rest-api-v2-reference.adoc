...
a|`POST /api/rest/2.0/connections/{connection_identifier}/revoke-refresh-tokens` +

Revokes OAuth refresh tokens for users who no longer require access to a Cloud Data Warehouse (CDW).

|ThoughtSpot Cloud: __26.2.0.cl or later__ +
a| +++<a href="{{navprefix}}/restV2-playground?apiResourceId=http%2Fapi-endpoints%2Fconnections%2Frevoke-refresh-tokens" id="preview-in-playground">Try it out</a>+++
...
a|`POST /api/rest/2.0/connections/{connection_identifier}/resync-metadata` +
...
a| +++<a href="{{navprefix}}/restV2-playground?apiResourceId=http%2Fapi-endpoints%2Fconnections%2Fsync-metadata" id="preview-in-playground">Try it out</a>+++
