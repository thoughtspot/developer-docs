...
To change the formula variable attributes of a user for a particular, particularly to set their entitlements to an empty set, use the `/api/rest/2.0/template/variables/update-values` API endpoint.
...
----
The above command would result in jane.smith@company.com being denied any access when `country_var` is used in an RLS rule.
