...
* xref:abac_rls-variables.adoc#_define_rls_rules_with_formula_variable[Adding RLS rules with formula variables] +
...
In this example, the formula references the `country_var` variable.
+
----
...
When the rule is translated, only the rows that match the specified condition are displayed to the user.

Wildcard “show all” via variables::
In this example, if the user’s region variable is set to `TS_WILDCARD_ALL`, they can view all regions; otherwise, they can see only their assigned region.
...
. Log in to your app and start the user session with the ABAC token.
