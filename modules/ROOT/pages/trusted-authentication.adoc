...
With access to the `xref:trusted-auth-secret-key.adoc[secret_key]`, back-end REST API processes can request a token for any user, then use the returned token as: 

* a login token using `session/login` create a long-lived session as that user
...
* link:https://github.com/thoughtspot/big-react-demo[Big React Demo, window=_blank]: React demo shows how to implement trusted authentication as part of an application integrated the various ThoughtSpot embedded components
* link:https://github.com/thoughtspot/thoughtspot_rest_api_v1_python[Python REST API library, window=_blank]: Library implementing the V1 and V2 REST APIs in Python
