...
You can create the following types of variables using variable API and use these variables to parameterize Connection or Table properties via metadata parameterization API:
...
Allows parameterizing connection properties per user or user group. It doesn't allow parameterizing core Connection properties such as `accountName`, `host`, or `port`. These properties must be derived from the connection configuration and cannot set via variables. +
This feature is disabled by default. To enable this option, contact ThoughtSpot Support.

The following figure illustrates how variable references in Connections and Table are replaced:
...
This step involves building Answers and Liveboard from a Model or data object in Primary Org. Ensure that the object references Tables or Connections that can be parameterized.
...
The steps required to publish objects from the Primary Org to other Orgs are illustrated in the following figure:
