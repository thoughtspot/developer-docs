...
Building content in ThoughtSpot for embedding is done through ThoughtSpot's intuitive UI, following the exact some processes for an internal business use case. 

Please see the link:https://docs.thoughtspot.com/cloud/latest/ts-cloud-getting-started[main ThoughtSpot documentation, target=_blank] to learn how to use ThoughtSpot to connect to and model data, perform searches and create Liveboareds.
...
The Visual Embed SDK provides a number of xref:getting-started.adoc[embedding components] for easy integration with the embedding app and control of the behavior of the ThoughtSpot component. Please see the xref:getting-started.adoc[embedding components guide] to get started.
