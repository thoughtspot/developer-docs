...
ThoughtSpot application components such as Liveboards, Answers, visualizations, Search and Spotter interface, and application pages support several actions and menu commands for user-initiated operations. To allow developers to customize the menu options and feature availability for their embedded application users, Visual Embed SDK provides action IDs for the actions available in ThoughtSpot application interface. Developers can use these action IDs in the `visibleActions`, `disabledActions`, or `hiddenActions` arrays to show, disable, or hide specific actions in the embedded view.

The availability of these actions may vary based on user privileges and the SDK library used for embedding. Please refer to to the following sections to know the action IDs available for each component.
...
=== Contextual menu
The following actions are available in the contextual menu of Liveboard visualizations. These actions are visible when a user right-clicks on a chart or table.
...
`AppEmbed`|*Exclude* +
Allows you to exclude a specific data point on a search answer. Available as a contextual menu action on the Answer.
|xref:Action.adoc#_drillinclude[`Action.DrillInclude`]|`LiveboardEmbed` +
`AppEmbed` |*Include* +
Allows you to include a specific data point on a search answer. Available as a contextual menu action on the Answer.
...
== Spotter
The following action IDs are available for Spotter interface:
...
`AppEmbed` | The pencil icon indicating the edit action on the Spotter prompt panel.
...
`AppEmbed`  | The delete icon indicating the delete action on the Spotter prompt panel.
...
Allows sending feedback about Spotter response.
|=====
...
`SearchEmbed` +
`SearchBarEmbed` +
`AppEmbed`| *Choose sources* option in the Search page and Spotter conversation panel +
...
=== Contextual menu
The following actions are available in contextual menu:
[width="100%" cols="3,3,4"]
[options='header']
|=====
|Action string in SDK| Required SDK library|Action label in the UI
