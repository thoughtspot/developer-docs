...
To enable *AI Highlights*, ensure that the link:https://docs.thoughtspot.com/software/latest/time-series-anomaly[KPI anomaly detection, window=_blank] feature is enabled on your instance.
...
`AppEmbed` | Action ID to disable or hide the filter visibility on Liveboard tabs. Applicable if link:https://docs.thoughtspot.com/software/latest/liveboard-filters#_apply_filters_to_specific_visualizations_or_tabs[the filter visibility on tabs, window=_blank] feature is enabled on your instance. +
