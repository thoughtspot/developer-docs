...
The xref:css-customization.adoc[ThoughtSpot CSS customization framework] defines a number of variables for applying styles throughout embedded ThoughSpot components. The framework supports customizing the following components and UI elements using the CSS variables:

* Application-wide settings such as background color of application pages, color and style of the text and font.
* Top and left navigation panel
* Menu elements on application pages. For example, for **More** menu image:./images/icon-more-10px.png[the more options menu], right-click contextual menu, and dropdown selection panels.
* Dialogs. The UI modals that appear on clicking an action. For example, the *Pin* action on an Answer page.
* Button elements. ThoughtSpot application contains the following types of button elements:

* Primary  +
Buttons that trigger an action for an entire page or a container. +
Primary buttons may contain text, text with an icon, or just an icon. For example, the *Save* button on the Answer page.
* Secondary  +
Buttons that trigger additional actions. For example, **More** menu image:./images/icon-more-10px.png[the more options menu].
* Tertiary  +
...
* Checkboxes
* Lists on pages such as *Liveboards*, *Answers*, *Schedules*, and *Data objects* and more.
* Visualization tiles, visualization layouts, groups and style elements in a Liveboard.
* Filter chips on the Answer or Liveboard page.
* Charts and tables in an Answer or visualization page +
Axis title, labels, style and color of charts, chart widget, chart selection icon.
* Spotter interface, prompt panel, conversation history panel elements.
* Search page and data panel experience
* Modular panels and elements on the *Home* page.

For a comprehensive list of CSS variables available for customization, see xref:CustomCssVariables[CSS variables reference].

== Using Theme Builder playground and CSS and interface customization
ThoughtSpot provides visual playground that allows you to try out the available customization options to adjust elements like button radius, background colors, fonts, and more for different application pages and interfaces. You can preview changes live, then export the configuration as JSON to apply directly in your embedded application or SDK.
...
|`--ts-var-liveboard-tile-border-color`| Border color of the tiles in the Liveboard.
|`--ts-var-liveboard-tile-background`| Background color of the tiles in the Liveboard.
|======
...
* xref:CustomCSSVariables.adoc[CSS variable reference]
