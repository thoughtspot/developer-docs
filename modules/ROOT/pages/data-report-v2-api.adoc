...
+
When `page_size = CONTINUOUS`, the `include_filter_page` option works to show/hide the filter section in the PDF page (in CONTINUOUS PDF, we don't have a separate filter page, but the filters are included on the same page at the top).
...
Contact ThoughtSpot support to enable these settings for PDF downloads on your ThoughtSpot instance.
