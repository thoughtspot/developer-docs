...
* However, due to a UI limitation within the API Playground, the `include_cover_page` and `include_filter_page` options will continue to be visible despite enabling the above settings. This should have no impact on your API operation.
* If these settings are not enabled, and you select the `include_cover_page` and `include_filter_page` options, the API will return an error. Due to a UI limitation within the API Playground and the `include_header` is automatically added to the payload and must be manually deleted from the JSON for the API call to work correctly.
====
