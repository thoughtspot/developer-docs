...
The SpotterCode MCP Server at `https://spottercode.thoughtspot.app/mcp` requires authentication. Authentication enables SpotterCode to make API calls to your ThoughtSpot instance on your behalf, which is required to use the `run-ts-workflow` skill for tasks such as generating secret keys, configuring CORS and CSP allowlists, and fetching object IDs.
...
=== MCP Server endpoint for documentation retrieval

If you only need documentation and REST API reference retrieval, use the following MCP Server URL which unauthenticated endpoint:
