...
The response to */metadata/search* can be parsed and split on *tags*, *author* or other properties, or you can make multiple calls to the endpoitn with the the various filtering arguments in each request. For example, xref:rest-api-v2-metadata-search.adoc#_favorite_object_options[favorite options] can be specified to only retrieve a lit of content that has been favorited.
