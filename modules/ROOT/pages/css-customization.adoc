...
----
+
However, if the order of the string changes as shown in the following example, the later string customization entry will take precedence; which means that the `Pin to Liveboard` will be customized as `Pin to Dashboard`, and the string definition for `Pin to Liveboard` is ignored.
...
. Identify the icon that you want to replace.
.. Right-click on the icon and select *Inspect*.
.. Inspect the `<svg>` element.
.. Note the icon ID.
. Create a sprite SVG file for icons and add the icon ID. For example, `symbol id="rd-icon-chart"`. +
To override more icons, add the icon IDs to the SVG file.
...
. To override the icons on the ThoughtSpot page, specify the icon sprite URL in the `iconSpriteURL` property of the `customziations` object in Visual Embed SDK.

+
The following example shows the code snippet to override the chart icon on the *Answers* page.
+
[source,HTML]
----
...
----
+
The following example shows how to add the SVG hosting URL for icon customization in the SDK.
+
[source,JavaScript]
----
 customizations: {
