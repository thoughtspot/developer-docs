...
CSS variables and selector overrides are defined using the `style` property of the `customizations` property.

You can use a CSS file to make your declarations of variables and rules with the `customCSSUrl` property or declare them inline within the Visual Embed SDK code using the `customCSS` property, which can contain a `variables` section and a `rules_UNSTABLE` section for selectors:
...
The `https://cdn.jsdelivr.net/` domain is allowed on every ThoughtSpot instance by default without additional configuration, for use of content available through the the jsdelivr CDN, including most GitHub content.
