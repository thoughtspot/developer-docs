...
In full app embed deployments, the SDK allows you to prevent embed application users from accessing ThoughtSpot application outside the context of your host app. You can set the `blockNonEmbedFullAppAccess` property in the SDK to allow or block user access to the application instance used for rendering embedded objects in your app. By default, the `blockNonEmbedFullAppAccess` property is set to `true`.
