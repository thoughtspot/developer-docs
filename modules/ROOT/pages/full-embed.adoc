...
ThoughtSpot administrators can enable the **New Navigation and Homepage experience** at the cluster level on the **Admin** > **Early access features**. When this feature is enabled on the cluster, users will see the **Navigation and Homepage** option on the **Profiles** page, so that users can switch to the Classic experience if required.
...
For more information about customizing navigation and UI experience for full application embedding, see xref:https://developers.thoughtspot.com/docs/full-app-customize[Customize full application embedding].
...
For more information full application embed object, customization of UI experience,  classes, methods, interface properties, and enumeration members, see the following pages:
