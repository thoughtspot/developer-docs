...
The `AppEmbed` component allows you to embed a specific application page or the entire application experience. If you are embedding full experience, you can xref:full-app-customize.adoc#_choose_the_page_to_load[set a specific application page as the default page when the embedded content loads] using the  `pageId` or `path` property.
