...
At any given time, only a single configuration can be customized for the notification emails of the Org. The last set configuration will be the active configuration.

[NOTE]
====
Customized configuration set for the Org overrides the configuration set for the ThoughtSpot instance. However, any Org with no specific configuration will reflect the same configuration as set for the ThoughtSpot instance.
====
