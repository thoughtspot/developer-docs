...
+
Note that the output is enclosed in quotation marks (").
. Remove the quotation marks and update the TML text as shown here: +
`guid: a162289a-c1ab-427e-9985-8fb5f7c7e539\nliveboard:\n  name: Liveboard 1\n`

. On the+++<a href="{{navprefix}}/restV2-playground?apiResourceId=http%2Fapi-endpoints%2Fmetadata%2Fimport-metadata-tml">**Metadata** > **Import Metadata TML** API page</a>+++, go to **body** > **metadata_tmls** parameter. +
...
----
. Replace `metadata_tmls3` with the text copied from step 2. Ensure that the text you copy doesn't include additional characters and quotation marks.
+
[source,JSON]
----
{
