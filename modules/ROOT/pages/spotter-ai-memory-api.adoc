...
** `type`: `RULE`, `RECIPE`, or `ALWAYS_APPLY_RULES`. If the type is `RULE`, the item includes the rule definition. If the type is `RECIPE`, the item includes the `user_query` and `recipe` fields.
...
The export memory API endpoint returns a YAML payload with a single top-level `memories` key holding a list of memory items. Each item includes the following object properties:
...
For `RECIPE` items, `content.recipe` is always a scalar string, not a nested YAML mapping. The server rejects any import where `recipe` is expressed as indented YAML keys.

Do not construct or modify `recipe` values manually. Always copy `recipe` content verbatim from an export. To adjust Spotter memory behavior, edit the `RULE` and `ALWAYS_APPLY_RULES` entries instead; these are plain text and designed for human editing.
====
