...
In ThoughtSpot, the link:https://docs.thoughtspot.com/software/latest/security-rls[Row Level Security Rules, window=_blank] are applied on Tables to restrict access to specific rows of data based on user or user group context. These rules ensure users only see the data they are permitted to access.

== Overview

RLS rules are link:https://docs.thoughtspot.com/software/latest/security-rls-implement[defined within ThoughtSpot, target=_blank] on Table objects, and automatically extend to all Models, saved answers, and Liveboards based on that Table.
...
For more information about the system variables and how RLS rules are applied using these variables, see the link:https://docs.thoughtspot.com/software/latest/security-rls-concept#_examples_of_rls_rules[RLS rules documentation, window=_blank].
...
To implement rules for different data types and attributes, you can use link:https://docs.thoughtspot.com/software/latest/rls-variables-reference[custom  variables].
...
Within ThoughtSpot, you will establish link:https://docs.thoughtspot.com/software/latest/tables-join#_create_a_table_join[INNER JOINs, target=_blank] between each of the columns of the ACL table and the fact or dimension table that needs to be filtered.
