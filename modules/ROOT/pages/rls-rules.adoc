...
If neither of these patterns can be implemented or if you want to implement fine-grained access control, consider using xref:rls-rules.adoc#_formula_variables[formula variables] and the xref:abac-user-parameters.adoc[ABAC via tokens] method.
...
Both *ts_username* and *ts_groups* are string values, so the RLS rule may require using casting functions on the columns within the database if those columns are not also string types. To implement rules for different data types and attributes, use the xref:formula-variables.adoc[formula variables]. You can also use a combination of formula variables and  xref:abac-user-parameters.adoc[ABAC token attributes] for granular access control and additional security filters.
...
* For information about formula variables, see xref:formula-variables.adoc[Formula variables].
