...
ThoughtSpot link:https://docs.thoughtspot.com/software/latest/security-rls[Row Level Security Rules, window=_blank] define how the user's username or the names of the groups they belong into WHERE clauses on the SQL queries generated.
...
== RLS rules overview
*RLS rules* are link:https://docs.thoughtspot.com/software/latest/security-rls-implement[defined within ThoughtSpot, target=_blank] on table objects, and automatically extend to all worksheets, saved answers, and Liveboards based on that table, every time.
