...
* *ts_username* - To apply rules to the user that's currently signed-in. +
...
Both *ts_username* and *ts_groups* are string values, so the RLS rule may require using casting functions on the columns within the database if those columns are not also string types. To implement rules for different data types and attributes, use the xref:formula-variables.adoc[formula variables]. You can also use a combination of formula variables and ABAC token attributes xref:abac-user-parameters.adoc[ABAC via tokens] for granular access control and additional security filters.
...
* xref:formula-variables.adoc[Formula variables]
* xref:variables.adoc[Variable APIs]
