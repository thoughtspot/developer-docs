...
Apart from the steps described in the following sections, you can generate an iFrame src URL using the `HostEvent.GetIframeUrl` in the Visual Embed Playground.

//This event is available in Visual Embed SDK v1.34.0 and can be used on ThoughtSpot instances with 10.10.0.sw or a later release version.
...
To load the object with a pre-selected Model, use the following URL format:
