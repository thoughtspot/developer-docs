...
* To embed the full ThoughtSpot application with all pages and top navigation bar, set the `primaryNavHidden` to `false`:
+
...
== Additional flags to customize the embedded view
...
You can also use the Visual Embed playground in the Develop tab to customize, enable, and disable features in the embedded view and inspect the code to copy the iFrame URL.
