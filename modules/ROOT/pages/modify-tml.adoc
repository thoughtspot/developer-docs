...
For more information about Connection TML and its limitations, see link:https://docs.thoughtspot.com/software/latest/tml-connections[TML for Connections, window=_blank].
...
Worksheets combine several *tables*, including *Views*, into a coherent data model optimized for searches. The link:https://docs.thoughtspot.com/software/latest/tml#syntax-worksheets[TML syntax for worksheets, window=_blank] defines all aspects of the *worksheet*, including the tables it joins together, the columns and their properties, filters, and so on.
