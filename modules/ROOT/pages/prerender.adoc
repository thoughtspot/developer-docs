...
If you do not want your host app to fetch any ThoughtSpot resources during its initial load, prerendering on demand is ideal.
...
Since we are not 'preloading' any assets or data, this strategy does not require any pre-render component. Simply pass a `preRenderId` in the `preRenderConfig` object of your normal component render.
