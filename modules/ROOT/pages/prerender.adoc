...
:page-title: Prerender for fast embedding
...
=== Mount the pre-rendered embed in a specific container
...
Since we are not _preloading_ any assets or data, this strategy does not require any pre-render component. Simply pass a prerender ID in the `preRenderConfig` object of your normal component render.
...
== Keep the pre-render configuration in sync with the actual embed
...
As a best practice, pre-render with the exact configuration you intend to display, the same liveboardId, settings, and runtime overrides. When the pre-rendered configuration matches, the SDK reuses the existing instance directly, resulting in a near-instant load.
