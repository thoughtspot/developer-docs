...
|`chart_color_palette`|No|Array of hex color values for charts. When provided with `REPLACE`, exactly 8 `primary` color entries must be specified in colors. `secondary` color entries, if provided, should be exactly 4 in number. If `secondary` shades are omitted, the server auto-generates 4 shades from the primary color.
...
|`default_logo`|No|Binary image file for the square application logo. Allowed formats are PNG, JPG, and JPEG only.
