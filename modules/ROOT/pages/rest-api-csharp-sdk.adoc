...
|`BearerToken`|`null`|[.version-badge.deprecated]#Deprecated# Static bearer token. Does not refresh. Requests fail with 401 after the token expires. Use `TokenProvider` or `CreateAsync` for automatic refresh instead.
...
== Authentication
The SDK supports the following authentication modes. The first three use automatic token management and require `CreateAsync`.

* xref:rest-api-csharp-sdk.adoc#username-and-password[Username and password]
* xref:rest-api-csharp-sdk.adoc#username-and-secretKey[Username and secret key]
* xref:rest-api-csharp-sdk.adoc#token-provider[Token provider]
* xref:rest-api-csharp-sdk.adoc#static-bearer-token[Static bearer token]
