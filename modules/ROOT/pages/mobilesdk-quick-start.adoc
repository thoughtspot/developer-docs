...
init({
  ...
  getAuthToken: async () => "full-auth-token" // Replace "full-auth-token" with logic to fetch the token from your backend.
  // For more code examples and implementation details, see https://developers.thoughtspot.com/docs/trusted-auth-sdk#_code_examples
});
----
