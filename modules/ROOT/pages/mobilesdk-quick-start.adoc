= Embed ThoughtSpot using React Native SDK
:toc: true
:toclevels: 2
...
const LiveboardEmbedView = () => {
  ...
  return (
    <>
      ...
      <LiveboardEmbed
        ref={webViewRef}
        liveboardId={liveboardId} // The ID of the Liveboard to embed
...
const LiveboardEmbedView = () => {
  ...
  return (
    <>
      <View>
        {/* Button to reload the Liveboard */}
        <TouchableOpacity onPress={reloadView}>
          <Text>Reload</Text>
        </TouchableOpacity>

        {/* Button to share the Liveboard */}
        <TouchableOpacity onPress={shareView}>
          <Text>Share</Text>
        </TouchableOpacity>
      </View>

      {/* Embed the Liveboard with a reference to control it */}
      <LiveboardEmbed
        ref={webViewRef}           // Assign the ref to control this component
        liveboardId={liveboardId} // Pass the Liveboard ID as a prop (make sure it's defined/imported)
      />
    </>
  );
};
...
Connect your device to the app, authenticate, and load the component.
...
Select a data point to drill down on a visualization or apply filters. Verify the event interactions and workflows.
