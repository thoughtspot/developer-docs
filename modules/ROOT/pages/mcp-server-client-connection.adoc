...
For OAuth clients, use the following URL:

`\https://agent.thoughtspot.app/mcp?api-version={YYYY-MM-DD}`

For OpenAI MCP and Responses API integration, you can use the following URL:
...
For clients that require a bearer token for authentication, use one of the following URLs:

* `\https://agent.thoughtspot.app/token/mcp?api-version={YYYY-MM-DD}` (Recommended) +
Uses the MCP Server version released on or before the specified date.

* `\https://agent.thoughtspot.app/token/mcp` +
Always points to the latest version. Use this with caution as it may introduce unintended changes to your environments.
