...
* Ensure that your setup has access to a ThoughtSpot application instance with 10.11.0.cl or a later release version. For MCP Server with Spotter 3 capabilities, ensure that your ThoughtSpot instance is 26.2.0.cl or later.
...
Currently, this URL points to the baseline version with legacy MCP tools. It will resolve to the latest version in the near future. During this period, if you want the MCP clients to use the latest tools, use the URL with the date parameter.
+
...
For more information about how to register a remote MCP Server, refer to your client's documentation.
...
. Verify that MCP tools are being called to generate responses. Refer to the xref:mcp-tool-reference-guide.adoc[MCP tool reference guide] for information about tool calls.
...
* Verify that the user session has expired. Log in to your ThoughtSpot application to start a new session.
