...
The second argument of the `LiveboardEmbed()` constructor is a `xref:LiveboardViewConfig.adoc[LiveboardViewConfig]` object. There are a number of properties that affect the Liveboard UI and even the display of the Liveboard itself.
...
To view specific data across the tables and charts on an embedded Liveboard, users can use Liveboard filter options. You can apply filters when embedding a Liveboard or xref:runtime-filters.adoc[apply runtime filters] via Visual Embed SDK and load the Liveboard with filters applied.
