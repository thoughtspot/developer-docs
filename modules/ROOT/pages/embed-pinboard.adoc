...
When enabled, the `enable2ColumnLayout` allows you to customize the Liveboard breakpoint width for embedded users.
...
To improve load performance for users who revisit the same Liveboard within a session, set `enableLiveboardDataCache` to `true`, so the embedded Liveboard caches visualization data in the browser.
...
When browser cache is enabled, the *Refresh* button appears in the Liveboard header. Users can click this button to clear the browser cache and fetch fresh data from the data warehouse on demand.

The Visual Embed SDK provides the following action and event IDs to customize the cache refresh visibility and workflow.
