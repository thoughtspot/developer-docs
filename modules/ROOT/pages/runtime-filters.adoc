...
Ensure that you add the runtime filter parameters before `#/path` in the URL, so that the parameters persist. For examples, see xref:runtime-filters.adoc#rtURLParams[Apply runtime filters on a Liveboard].
====
...
[#rtURLParams]
== Apply runtime filters on a Liveboard
