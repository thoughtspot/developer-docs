...
`SearchEmbed` using the `runtimeFilters` property in the Visual Embed SDK, ThoughtSpot applies a configurable limit. The default and maximum value is 1000 filters.

ThoughtSpot administrators can configure this limit for their instance. The value must be between 1 and 1000.
To check or update the configured limit, contact your ThoughtSpot administrator.
