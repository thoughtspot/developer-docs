...
+
* *Smart merge import*: The import process now applies merge logic instead of overwriting. Views present only in the destination are preserved; views present only in the import are appended; shared views are updated.
...
+
// TODO: verify with engineering — confirm the exact TML field names and data types for `owned_by` and `obj_id`; confirm whether `obj_id` is auto-generated on first export or must be specified.
...
+
// TODO: verify with engineering — confirm the exact endpoint path for the Update Metadata Object ID API and the request schema for LOGICAL_COLLECTION.
...
// SOURCE: SCAL-273333
+
The `Can Download Data` privilege is replaced by two new granular privileges. Update privilege assignments in user and group management requests to use the new values:
+
...
Schedule attachment size limit corrected::
// SOURCE: SCAL-319440
+
The maximum supported attachment size for communication channel schedules is corrected to *22 MB*. The previous value of 25 MB was inaccurate due to base64 encoding overhead in email delivery.
+
The following endpoints are affected:
+
* `POST /api/rest/2.0/schedules/create`
