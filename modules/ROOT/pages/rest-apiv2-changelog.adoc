...
Personalized Views — smart merge, `owned_by`, and `obj_id`::
// SOURCE: SCAL-307283
+
...
+
* *Smart merge import*: The import process now applies merge logic instead of overwriting all existing views. Views present only in the destination environment are preserved; views present only in the import payload are appended; views present in both are updated.
...
+
// TODO: verify with engineering — confirm the exact TML field names and data types for `owned_by` and `obj_id`; confirm whether `obj_id` is auto-generated on first export or must be specified manually.
...
+
The Update Metadata Object ID API also accepts `LOGICAL_COLLECTION` as a valid object type.
+
...
// SOURCE: SCAL-273333
+
The `Can Download Data` privilege is replaced by two new granular privileges. Update privilege assignments in user and group management API calls to use the new values:
+
...
// SOURCE: SCAL-244749
+
The `MINUTES` frequency option (every N minutes) for Liveboard schedules is no longer supported. Requests to create or update a schedule using a minutes-based frequency will be rejected.
+
...
// SOURCE: SCAL-319440
+
The documented maximum attachment size for communication channel schedules is corrected to *22 MB*. The previous value of 25 MB was inaccurate due to base64 encoding overhead in email delivery, which increases the effective payload size by approximately 33%.
+
The following endpoints are affected:
+
* `POST /api/rest/2.0/schedules/create`
