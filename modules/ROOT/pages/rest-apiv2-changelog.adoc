...
This release introduces the following new REST API v2.0 endpoints for programmatic feature management. All endpoints are under `/api/rest/2.0/configurations/features/`.
...
| Returns feature configurations grouped by feature group.

| `POST /api/rest/2.0/configurations/features/assignments/update`
| Updates Org assignments for a feature using `ADD`, `REMOVE`, or `REPLACE` operations.

| `POST /api/rest/2.0/configurations/features/values/update`
| Sets feature value at `CLUSTER` or `ORG` scope.
|===
