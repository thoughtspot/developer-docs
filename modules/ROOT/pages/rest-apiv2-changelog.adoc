...
=== Security settings APIs [beta betaBackground]^Beta^
This release introduces the following Security settings APIs

* `POST /api/rest/2.0/system/security-settings/configure` +
Allows configuring security settings for ThoughtSpot instance or for an Org to ensure data security and a seamless user experience in embedding applications.
* `POST /api/rest/2.0/system/security-settings/search` +
Gets the security settings configuration for the ThoughtSpot instance, or for a particular Org, or for both.

For more information, see xref:security-settings.adoc[Security Settings].

=== Export Liveboard Report APIs
You can now also export Liveboard reports in the .csv [earlyAccess eaBackground]#Early Access# and .xlxs [earlyAccess eaBackground]#Early Access# format through the `POST /api/rest/2.0/report/liveboard` API endpoint.

For more information, see xref:data-report-v2-api.adoc[Data and Report APIs].

=== Customize email template APIs

The `template_properties` parameter has two additional elements for email template customization:
