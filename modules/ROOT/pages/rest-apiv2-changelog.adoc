...
=== Report APIs [tag redBackground]#BREAKING CHANGE#
The `POST /api/rest/2.0/report/answer` and `POST /api/rest/2.0/report/liveboard` no longer support downloading objects in CSV and XLS format. These endpoints support downloading data only in the PDF and PNG formats.
...
=== New Export options for TML export
The `/api/rest/2.0/metadata/tml/export` endpoint now allows additional options to include when exporting TML for an object from ThoughtSpot. The `export_options` allows you to include the following optional attributes:

* `include_obj_id_ref` +
Specifies whether to export `user_defined_id` of referenced object. This setting is valid only if the `UserDefinedId` property in TML is enabled.
* `include_guid` +
...
The `/api/rest/2.0/auth/token/custom` API endpoint now allows reset option to specify if the attributes assigned to the token should persist or be reset.
...
The response body from `POST /api/rest/2.0/ai/answer/create` and `POST /api/rest/2.0/ai/conversation/{conversation_identifier}/converse` shows the `display_tokens` property.
