...
== Version 10.9.0.cl, May 2025

=== Metadata parameterization and content publishing across Orgs [beta betaBackground]^Beta^

This release introduces the following new endpoints for metadata parameterization and content publishing across Orgs. These APIs are in the beta. To enable these APIs and the content publishing feature on your instance, contact ThoughtSpot Support.
...
If your metadata objects are parameterized, you can also use the `show_resolved_parameters` to filter the API response from `/api/rest/2.0/connection/search` and `/api/rest/2.0/metadata/search` endpoints to get only the objects with resolved parameterized values.

=== Liveboard Report API
The Liveboard Report API now allows you ti define the following properties:
...
Optional parameter to specify the name or GUID of the Liveboard personalized view that you want to download.
