...
The `POST /api/rest/2.0/report/answer` endpoint is now GA with the following enhancements:

Pinned Answer export::
Set `type` to `PINNED` to export a pinned Answer (a visualization on a Liveboard) directly. The `metadata_identifier` must be the parent Liveboard GUID or name (the containerId), and the Answer downloads with Liveboard context: Liveboard-level filters, Runtime Filters, and Column Security Rules are applied automatically. If `type` is not specified, it defaults to `SAVED`.
...
Display scaling::
Use `scaling` (range: 80–500%) to adjust the relative size of chart elements in a PNG export without cropping the image.
...
Administrators can assign the Scheduled Liveboards process to a connection configuration, enabling ThoughtSpot to use the associated credentials when running scheduled Liveboard delivery jobs.
