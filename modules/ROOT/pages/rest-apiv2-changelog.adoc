...
=== Export Metadata TML enhancements [beta betaBackground]^Beta^
The  `export_options` in the export metadata TML `POST /api/rest/2.0/metadata/tml/export` API now includes an additional boolean flag `export_column_security_rules` which indicates whether to export the column security rules of the object.
