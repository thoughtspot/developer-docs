...
* `commit_branch_name` +
Allows configuring the commit branch for Git connections on your ThoughtSpot instance. ThoughtSpot recommends using `commit_branch_name` instead of `default_branch_name` in the API call to prevent users from committing changes to default deployment branch.
* `configuration_branch_name`
+
Allows defining a separate Git branch for storing and maintaining configuration files, such as GUID mapping file and commit tracking file. If the `configuration_branch_name` property is defined, `guid_mapping_branch_name` is not required.

Modified parameters::
The `enable_guid_mapping` parameter is enabled by default.

Separate branches for Orgs::
If you are using Orgs and want to move content between these Orgs using version control APIs, ensure that you use a separate Git branch for each Org. If two Orgs are connected to the same Git `repository_url`, the `POST /api/rest/2.0/vcs/git/config/create` and `POST /api/rest/2.0/vcs/git/config/update` API endpoints do not support configuring the same branch name for these Orgs.
...
For more information, see xref:version_control.adoc#connectTS[Git integration and version control].
...
For more information, see xref:version_control.adoc#_commit_files_and_changes[Git integration and version control].
...
For more information, see xref:version_control.adoc#_deploy_commits[Git integration and version control].
