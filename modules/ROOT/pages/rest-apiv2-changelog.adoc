...
* Table TML: +
The `POST /api/rest/2.0/metadata/tml/export` API requests allows exporting column security rules for Table TML objects. This attribute will export column security rules only if the object specified in the API request has column security applied and when `export_associated` is set to `true`.
