...
This page describes how to use AI APIs with Spotter Classic to query and explore data through conversational interactions.
...
|`metadata_identifier`|_String_. Required. Specify the GUID of the data source object or model. The metadata object specified in the API request will be used as a data source for the conversation.
|`tokens` +
...
|`conversation_identifier`|Path parameter|__String__. Required. Specify the GUID of the conversation received from the xref:spotter-classic-apis.adoc#_create_a_conversation_session[create conversation API call].
