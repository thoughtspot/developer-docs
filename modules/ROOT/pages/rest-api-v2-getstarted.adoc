...
* To trusted authentication to authenticate REST clients, make sure xref:trusted-auth-secret-key.adoc#trusted-auth-enable[Trusted authentication is enabled] on your ThoughtSpot instance, and your authenticator service has access to the `secret key`.
