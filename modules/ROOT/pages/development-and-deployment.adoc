...
In a scenario where end customer databases are single-tenanted, the final deployment step is to the *publisher* Org, updating the *Published* objects, which instantly updates the objects in every Org the objects are published to.

=== Publishing
*xref:publishing-overview.adoc[Publishing]* makes objects available in other Orgs without making copies. Variables can be set at the Org level to override the Connection and Table object details for Publishing objects when they are accessed in a specific Org.
