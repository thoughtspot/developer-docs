...
These settings can also be done via the **Security Settings** page in the ThoughtSpot UI, or through REST APIs v2, by sending a `POST` request to `POST /api/rest/2.0/system/security-settings/configure` API endpoint.
  ...
  "org_preferences": [
    {
      "cors_whitelisted_urls": [
        "cors_whitelisted_urls2"
      ]
    }
  ],
  "cluster_preferences": {
    "cors_whitelisted_urls": [
      "cors_whitelisted_urls1"
    ]
  }
}'
----
