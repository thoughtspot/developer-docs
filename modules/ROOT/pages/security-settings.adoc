...
If you have embedded ThoughtSpot content in your app, you may want your users to access ThoughtSpot pages only within the context of your host app.

By default, the embedded application users cannot access or switch to the ThoughtSpot application experience in the non-embed mode. However, users with administrator or developer privileges can access ThoughtSpot in the non-embed mode. They can also enable or turn off the *Block non-embed full app access* feature on the *Develop* > *Customizations* > *Security Settings* page. The Visual Embed SDK also supports blocking user access. For more information, see xref:full-embed.adoc#_block_access_to_non_embedded_thoughtspot_instance[Block access to non-embedded ThoughtSpot application context].
