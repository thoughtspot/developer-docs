...
** CORS hosts — The UI allows adding a domain URL with the protocol (`http/https`). However, ThoughtSpot ignores the protocol in the CORS host domain. Therefore, you can exclude the protocol in the domain URL strings.
** For localhost and non-HTTPS URLs — For non-HTTPs domains or localhost such as `localhost:3000`, if you add the domain without the protocol, the `https` protocol will be used by default. Due to this, the localhost domain with `http` (`\http://localhost:3000`) might result in a CSP or CORS error. Therefore, include the `http` protocol in the domain name strings for non-HTTPS domains and localhost.
