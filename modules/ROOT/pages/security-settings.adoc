...
----
    ...
    // SOURCE: SCAL-320438
    // TODO: [WRITER] Confirm with Prashant Borkar whether this section belongs under
    //       "CSP allowlists" or as a standalone top-level section under "Configure security settings".
    //       Also confirm the exact REST API field name for adding custom schemes via
    //       POST /api/rest/2.0/system/security-settings/configure.
    // TODO: [WRITER] Confirm the full list of supported custom schemes beyond
    //       capacitor:// and ionic:// with the engineering team.

    [#custom-app-schemes]
    ==== Allow custom app schemes for mobile and hybrid embeds

    [.badge.badge--info]#ThoughtSpot Cloud 26.8.0.cl#
    ...
    [IMPORTANT]
    ====
    *Security caveat — understand before allowlisting custom schemes*

    Allowlisting a shared app scheme such as `capacitor://localhost` or `ionic://localhost` does *not* uniquely identify your application. Any application on the same device that uses the same framework presents the same origin to the browser. This means:
    ...
    * Do *not* rely on origin allowlisting alone as a security mechanism for custom-scheme embeds.
    ...
    // TODO: [WRITER] Confirm the exact request body field name for custom scheme CSP hosts
    //       in POST /api/rest/2.0/system/security-settings/configure with Prashant Borkar.
    //       The field may be `visual_embed_hosts` (same as standard CSP hosts) or a separate field.

    Send a `POST` request to the `POST /api/rest/2.0/system/security-settings/configure` endpoint and add the custom scheme URL to the `visual_embed_hosts` and `cors_whitelisted_urls` arrays:
...
* **Websocket endpoints**: +
You can add Websocket (`wss://`) endpoints for external tool script integrations, for example, tools that open WebSocket connections from the browser. Only hosts explicitly listed with `wss://` are permitted.
====
...
a|Websocket URLs +
`wss://`| [tag greenBackground tick]#✓# Supported |[tag greenBackground tick]#✓# Supported |[tag redBackground tick]#x# Not Supported| [tag greenBackground tick]#✓# Supported
|
|====
...
To enable or disable trusted authentication at the cluster or Org level, send a request to the `POST /api/rest/2.0/auth/configure` endpoint.

To find the trusted authentication configuration for the specified auth type at the cluster or Org level, send a request to the `POST /api/rest/2.0/auth/search` endpoint.
