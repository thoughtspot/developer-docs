...
If you plan to create custom actions with URL targets or use a webhook to send data to an external app or endpoint, you must add the domain names of these URLs to the `CSP connect-src` allowlist. This allows JavaScript events triggered by the custom action URLs.
