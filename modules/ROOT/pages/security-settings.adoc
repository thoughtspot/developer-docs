...
These settings can also be done via REST APIs v2, by sending a `POST` request to `POST /api/rest/2.0/system/security-settings/configure` API endpoint.
...
== Configure security settings via the ThoughtSpot UI
...
#Through the API::
`cors_whitelisted_urls` is an array of whitelisted URLs. Set this to `null` when there is no urls to be set.#
...
== Configure security settings via API
#
...
Security settings can be configured for the ThoughtSpot instance and for the Org as well. To configure security settings send a `POST` request to `POST /api/rest/2.0/system/security-settings/configure` API endpoint.
...
|cluster_preferences a|__String__. Required. A JSON map of customizable security settings for the ThoughtSpot instance.
|enable_partitioned_cookies a|__Boolean__.
