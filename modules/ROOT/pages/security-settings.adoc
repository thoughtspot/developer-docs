...
==== Complete user access restriction
...
==== Selective user access restriction
There is now a privilege for users who need to access the non-embedded ThoughtSpot pages from their embedded app context. Adding the ThoughtSpot user to a group with role privilege ALLOW_NON_EMBED_FULL_APP_ACCESS will allow the user to access ThoughtSpot cluster even when `blockNonEmbedFullAppAccess = TRUE`, just like the administrator and developer.
