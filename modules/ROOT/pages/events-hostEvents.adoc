...
* For information about triggering events on React components, see xref:react-components_lesson-04.adoc[Event listeners for React components].
    ...
    | `HostEvent.GetGroups`
    | 26.10.0.cl
    | Returns filter and parameter group details for the current Liveboard. Response includes `orderedGroupIds`, `numberOfGroups`, and `Groups`. Mirrors `HostEvent.GetTabs`.

    | `HostEvent.OpenParameter`
    | 26.10.0.cl
    | Opens the parameter panel for a specific parameter on the Liveboard. Accepts an optional `applicability` object to scope the action to a tab or group. Mirrors `HostEvent.OpenFilter`.
    |===
