...
[NOTE]
====
You must specify at least one API type or URL in the array for interception to be effective. If `interceptUrls` is set as an empty array, no URLs will be intercepted, and the API intercept feature will not trigger for any API calls even if `enableApiIntercept` is set to `true`.
====
...
LiveboardEmbed.on(EmbedEvent.ApiIntercept, (payload, responder) => {
    ...
    responder({
        data: {
            ...
            error: {
                errorText: 'Error Occurred', // Provide a simple error message to the user.
            }
        }
    })
})
