...
+
With the new Data panel experience, you can collapse the search bar preventing it from taking the entire iframe space if more tokens are added, using `collapseSearchBarInitially`.
+
All the functionalities in the *+Add* menu now have associated actions.
...
Starting 10.3.0.cl, the data panel on an embedded Search page for new users is presented in the new experience mode. The existing users can enable the new data panel experience either via ThoughtSpot UI or by setting the `dataPanelV2` property to `true` in the SDK.
