...
--
[IMPORTANT]
*Deprecation notice — 26.8.0.cl:* The classic (V1) experience and V2 experience modes are deprecated as of ThoughtSpot Cloud 26.8.0.cl. All embedded ThoughtSpot applications now render the V3 navigation experience by default. SDK properties targeting V1 or V2 navigation are silently ignored. Update your `AppViewConfig` to use `discoveryExperience` with `primaryNavbarVersion: PrimaryNavbarVersion.Sliding`. For migration guidance, see xref:full-app-customize.adoc[Customize full application embedding].
...
|Classic (V1) experience [.version-badge.deprecated]#Deprecated# a|A standard top navigation bar with the following components: +

* A horizontal application menu
* Help and user profile icons
* Org switcher

|V2 experience [.version-badge.deprecated]#Deprecated# a|
...
// NOTE TO WRITER: The original file was truncated after this point. Please verify and restore the remaining content (examples and any additional sections) from the source file on the main branch before publishing.
