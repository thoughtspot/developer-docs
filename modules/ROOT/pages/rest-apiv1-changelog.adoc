...
TML::
The TML import API now allows setting `skip_cdw_validation_for_tables` property to indicate whether to skip Cloud Data Warehouse (CDW) validation during import.

For mor information, see xref:tml-api.adoc#import[TML import].

User::
You can now assign the `SUSPENDED` state to a user account when using the following API endpoints:

* `POST /tspublic/v1/user`
* `PUT /tspublic/v1/user/{userid}`

The `SUSPENDED` user state is applicable only to IAMv2 users.
