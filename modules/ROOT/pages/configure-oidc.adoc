...
You need admin privileges to enable OIDC authentication with IAMv2.
...
To link:https://docs.thoughtspot.com/cloud/9.10.0.cl/saml-okta#_enable_saml_authentication[establish a OIDC authentication connection using IAMv2, window_=blank], the *OIDC IDP* tile needs to be selected from the IdP options in the *Admin* panel.
For a successful connection, the IdP details need to be provided and mapped with the corresponding ThoughtSpot attribute. The following IdP details are to be provided:
...
These IdP details will have to be mapped with the corresponding ThoughtSpot attributes:
...
For detailed information on enabling OIDC authentication on ThoughtSpot using IAMv2, and attributes, follow the steps listed on this page link:https://docs.thoughtspot.com/cloud/9.10.0.cl/oidc-iamv2#_enable_oidc_authentication[Enable OIDC authentication, window=_blank].
