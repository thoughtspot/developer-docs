...
[NOTE]
====
In ABAC implementation, you can assign the variable values during token generation. Before configuring the token properties and variable values, verify the variables and their assignment status using the xref:variables.adoc#_get_variables[variable search API request].
====
...
.5+|`variable_value_scope` 2+| Set the scope for variable values. These properties determine the entity level, such as Org, user, or user-group, for which the values will apply.
...
For example, if a variable is configured for both the user and their group, the system determines which value to based on the assigned priority.
||
|=====
...
If the formula variable update is successful, you can use these variables in link:https://docs.thoughtspot.com/software/latest/rls-variables-reference[RLS rules, window=_blank] and xref:abac_rls-variables.adoc[ABAC via RLS with variables].
...
* For more information about using formula variables in RLS rules, see link:https://docs.thoughtspot.com/software/latest/rls-variables-reference[Formula variables, window=_blank].
...
* For information about RLS rules, see link:https://docs.thoughtspot.com/software/latest/security-rls[RLS rules documentation, window=_blank].
