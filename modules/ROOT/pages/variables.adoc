...
== Overview
You can configure variables programmatically via REST APIs, and use these variables for the following purposes:
...
Allows adding, removing, or replacing values of one or more variables configured in ThoughtSpot.
...
|`variable_identifier` a| __Array of strings__. Specify the variable identifiers to which you want to assign values.
...
For example, if both a user and their group have values assigned for the same variable, the system uses the assigned priority to decide which value to apply.
||
|=====
  ...
  "variable_value_scope": [
    {
      ...
      "model_identifier": "Sample retail sales"
    }
  ]
}'
----
...
If the variable is used by other objects, make sure to update or remove those references before deleting the variable.
