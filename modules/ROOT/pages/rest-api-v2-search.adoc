...
== Search users
You can use `metadata/search` with the `type` set to `USER` to get a basic listing, but the `users/search` endpoint provides many more filtering options that are specific to the properties of user objects.

For more information about this API, see xref:rest-api-v2-users-search.adoc[Search users].

== Search groups
You can use `groups/search` with the `type` set to `USER_GROUP` to get a basic listing, but the  `groups/search` endpoint provides many more filtering options that are specific to the properties of user group objects.
