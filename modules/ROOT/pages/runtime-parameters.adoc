...
Parameters in ThoughtSpot are typically used for "what-if" analysis, scenario planning, or to personalize data views without modifying the underlying data object. You can link:https://docs.thoughtspot.com/software/latest/parameters-create[create Parameters^] in a Model and integrate them into formulas, filters, data queries, and Liveboards. You can also link:https://docs.thoughtspot.com/software/latest/parameters-use[use Parameters^] within formulas when querying your data via Search, Liveboards, Answers, or Spotter sessions.

[NOTE]
====
Runtime Parameters is not a security feature. To implement data security rules in ThoughtSpot, please use xref:rls-rules.adoc[Row Level Security (RLS rules)].
====
