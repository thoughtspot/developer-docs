...
Use the `hiddenHomepageModules` property to hide specific modules on the home page. The allowed values for `hiddenHomepageModules` are listed in the following table:
...
| `HomepageModule.SpotIQAnalysis` +
To show or hide the *SpotIQ Analysis* module.
| [tag redBackground tick]#x# Not supported
| [tag greenBackground tick]#✓#  Supported
| [tag greenBackground tick]#✓#  Supported

| `HomepageModule.Recents` +
To show or hide the *Recently viewed* module.
| [tag redBackground tick]#x# Not supported
| [tag greenBackground tick]#✓#  Supported
| [tag greenBackground tick]#✓#  Supported
|===

// NOTE TO WRITER: Verify the complete list of HomepageModule enum values above is accurate for 26.8.0.cl.
// The original file may have additional values — confirm against the SDK HomepageModule enum definition.
