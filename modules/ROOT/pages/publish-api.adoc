= Publish content to Orgs via APIs
:toc: true
:toclevels: 2

:page-title: Publish content to Orgs
...
To publish objects programmatically from the Primary Org to one or several target Orgs in a single API call, use the `POST /api/rest/2.0/security/metadata/publish` API endpoint.
...
To publish an object to one or several Orgs, use the `/api/rest/2.0/security/metadata/publish` API endpoint.
...
|`metadata` a| __Array of strings__. Array of metadata objects to publish.

Include the following parameters for each entry in the `metadata` array:

* `identifier`: Specify the ID and type of metadata in each array.
* `type`: Specify one of the following values:
**  `LIVEBOARD` for Liveboards
** `LOGICAL_TABLES` for Models and Tables
** `ANSWER` for Answers
...
To remove published objects from the target Orgs, use `POST /api/rest/2.0/security/metadata/unpublish` API endpoint.
...
|`metadata` a|__Array of strings__. Array of the published objects to remove from the Orgs.

Include the following parameters for each entry in the `metadata` array:

* `identifier`: Specify the ID and type of metadata.
* `type`: Specify one of the following values:
**  `LIVEBOARD` for Liveboards
** `LOGICAL_TABLES` for Models and Tables
** `ANSWER` for Answers
