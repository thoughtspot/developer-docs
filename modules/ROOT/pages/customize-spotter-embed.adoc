...
Spotter 3 experience introduces a new prompt interface that includes additional features and user elements to enrich your Spotter experience.
...
== Spotter conversation sharing
If you are using Viusal Embed SDK 1.52.0 or later, your application users can share saved Spotter conversations with other users and groups from the embedded Spotter interface. Recipients get view access to the shared conversation.

=== Disable conversation sharing
To turn off conversation sharing in the embedded view, set `disableSpotterConversationSharing` to `true`:

[source,JavaScript]
----
const spotterEmbed = new SpotterEmbed('#ts-embed', {
  worksheetId: '<datasource-guid>',
  disableSpotterConversationSharing: true,
});
----

To hide only the *Share* button while keeping sharing available through other flows, use the action ID:

[source,JavaScript]
----
hiddenActions: [Action.ShareSpotterConversation],
----

=== Events

* `HostEvent.ShareSpotterConversation`, open the share dialog from your application:
+
[source,JavaScript]
----
...
* `EmbedEvent.SpotterConversationShared`, notified when a share completes:
+
[source,JavaScript]
----
spotterEmbed.on(EmbedEvent.SpotterConversationShared, (payload) => {
  // payload includes the conversation ID and shared principals
});
----
