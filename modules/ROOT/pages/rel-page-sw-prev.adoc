...
:page-title: Release Notes for previous versions
:page-pageid: rel-notes-prev
:page-description: ThoughtSpot Embedded release notes for ThoughtSpot Software releases.

This page provides information about the new features and enhancements introduced in the previous versions of ThoughtSpot Software for REST APIs and embedding ThoughtSpot. For information about the changes introduced in a new release, see xref:rel-page-sw.adoc[What's new].
...
This feature is turned off by default. To enable this feature on your ThoughtSpot instance, contact link:https://community.thoughtspot.com/customers/s/contactsupport[ThoughtSpot Support, window=_blank]. For more information, see xref:orgs.adoc[Orgs context for sharing links].
...
* The `/tspublic/v1/metadata/tml/export` API endpoint supports exporting FQNs of TML objects. To export FQNs, you must set the `export_fqn` property to true in your API request.
+
...
You can also access the REST API v1 and REST API v2 [beta blueBackground]^BETA^ Playground to explore request and response workflows. The REST API v2 [beta blueBackground]^BETA^ Playground provides an interactive code panel to explore the API request and response workflows, build code samples, and view API documentation.
