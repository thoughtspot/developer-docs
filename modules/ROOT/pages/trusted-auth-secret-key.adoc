...
. Click *Edit*.
. To enable trusted authentication, turn on the *Trusted authentication* toggle. #To enable Trusted authentication programmatically, send a `POST` request to the `POST /api/rest/2.0/auth/configure` endpoint endpoint. For more information, see xref:authentication.adoc[Configuring authentication settings].#
