...
access to Org A, Org B and Org C. While the user is logged in Org A, it can access a Liveboard which has been shared from Org C without having to switch the Org.
...
`\https://{ThoughtSpot-Host}/#/?OrgID/{object-id}/{sub-object-id}`
