...
[IMPORTANT]
====
* The per Org URL feature will not work if the developer has enabled the sdk settings for trusted authentication(`AuthType.TrustedAuthToken`) or cookieless authentication(`AuthType.TrustedAuthTokenCookieless`) in a parallel tab.
