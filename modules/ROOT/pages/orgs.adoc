...
* The `overrideOrgId` parameter in the SDK  accepts the Org ID from the resulting URL as its input value.
* The `overrideOrgId` parameter may not work properly with auth types - trusted authentication(`AuthType.TrustedAuthToken`) or cookieless authentication(`AuthType.TrustedAuthTokenCookieless`) simultaneously in the same browser, if the `tokenAuthPerOrg` is already enabled on the ThoughtSpot instance.
* Currently, there is no support for this feature through the APIs.
====
