...
Starting 10.5.0.cl, ThoughtSpot embedded users assigned to multiple Orgs will seamlessly access content from a different Org, while trying to access it from another Org.

To enable this feature on your instance, contact link:https://community.thoughtspot.com/customers/s/contactsupport[ThoughSpot Support, window=_blank]. Once this feature is enabled, the Org id can be specified in the ThoughtSpot object URL as query parameters.
