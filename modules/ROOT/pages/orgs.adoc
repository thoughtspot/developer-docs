...
For OIDC authentication, ensure that the Org support is enabled for the ThoughtSpot cluster. For more information, see link:https://docs.thoughtspot.com/cloud/latest/authentication-integration[ThoughtSpot Product Documentation].
...
* If xref:configure-oidc.adoc#orgMapping[Orgs support is enabled for OIDC authentication], and the Org objects to which the user belongs are configured on ThoughtSpot:
