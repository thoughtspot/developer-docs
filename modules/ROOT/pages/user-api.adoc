...
[NOTE]
====
Customers must set a password during user creation if they want to avoid an activation email from being sent.
====
...
[NOTE]
====
The IAMv2 administrators have an additional user state, `SUSPENDED` in the user properties. For more information, see xref:https://developers.thoughtspot.com/docs/api-user-management[User migration to IAMv2]
====
...
[NOTE]
====
The `content` JSON map allows administrators to update the user account status to states such as `ACTIVE`, or `PENDING`. IAMv2 users also have the `SUSPENDED` user state. For more information, see xref:https://developers.thoughtspot.com/docs/api-user-management[User migration to IAMv2]
====
