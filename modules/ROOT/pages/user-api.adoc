...
|`sendEmail` | Query parameter|__Boolean__. When set to `true`, a welcome email is to the user's email address.
|===
=== Example request
...
To de-activate a user account, send a `POST` request to the `/tspublic/v1/user/inactivate` API endpoint.
...
|===
|Form parameter|Description
|`username`|__String__. Username of the user account to de-activate.
...
If the de-activation operation is successful, the API returns the following response:
...
[IMPORTANT]
====
When a user account is de-activated using the `/tspublic/v1/user/inactivate` endpoint, the API returns a response with the user ID and token string. Use this ID and the token string along with user properties in the base64 encoded format in the API request to re-activate the user account.
====
