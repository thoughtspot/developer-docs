...
|`properties` +
__Optional__|Form parameter a|__String__. A JSON map of user properties such as email address. +
`"properties": {
...
|`usertype` +
__Optional__ a|Form parameter a|__String__. Type of user. The default value is `LOCAL_USER`, which indicates that the user is created locally in the ThoughtSpot system. Valid values include:
...
|`tenantid` +
__Optional__|Form parameter|__String__. GUID of the tenant for which the user is being created.
