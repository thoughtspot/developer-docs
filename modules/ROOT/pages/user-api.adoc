...
[NOTE]
====
The IAMv2 users have an additional user state, `SUSPENDED`. For more information, see xref:https://developers.thoughtspot.com/docs/api-user-management[User migration to IAMv2]
====
...
|`usertype` __Optional__ a|Form parameter a|__String__. Type of user. The default value is `LOCAL_USER`, which indicates that the user is created locally in the ThoughtSpot system. For IAMv2 users, the default value will be `OIDC_USER`. Valid values include:
...
NOTE: If you call this API when importing user properties from an external server, such as Active Directory or IdP, the `usertype` property is automatically assigned to the users. For example, a user from an LDAP server is added as `LDAP_USER` in ThoughtSpot.
...
[NOTE]
====
The IAMv2 users have the option to transition from `SUSPENDED` to `ACTIVE` or `PENDING` user state. For more information, see xref:https://developers.thoughtspot.com/docs/api-user-management[User migration to IAMv2]
====
