...
[NOTE]
====
The legacy GitHub REST APIs referenced here are tied to GitHub exclusively. The newer xref:git-provider-integration.adoic[Git provider integration pattern] provides more flexibility and control and should be utilized if you are starting out or having issues with the GitHub APIs.
====
...
The following example shows the API request with Liveboard and Model objects to commit to Git.
