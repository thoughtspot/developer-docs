...
The `/api/rest/2.0/vcs/git/commits/deploy` endpoint imports the TML files from the last commit in a Git branch back into the linked ThoughtSpot Org.

The API will deploy the head of the branch unless a `commit_id` is specified in the API request.
