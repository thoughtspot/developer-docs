...
An link:https://github.com/thoughtspot/thoughtspot_rest_api_v1_python/blob/main/examples_v2/git_deploy_commits_to_prod_single_tenants.py[example script in Python, target=_blank] is available showing the pattern of deploying to "prod org per customer" from a single `pre_prod/release` branch, to avoid the need for pull requests into each branch linked to an end customer Org. 
