...
Each environment is a linked pair of a ThoughtSpot Org and a Git branch that can only export (xref:git-rest-api-guide.adoc#_commit_files[commit]) or import (xref:git-rest-api-guide.adoc#_deploy_commits[deploy commits]).
...
Both the xref:git-rest-api-guide.adoc#_deploy_commits[deploy] and the xref:git-rest-api-guide.adoc#_validate_merge[validate merge] APIs can be used in modes that check for errors before doing merge and deploy actions.
...
The xref:access-control-sharing.adoc#_sharing_via_rest_api[REST API for sharing objects] can be called on the newly created objects. All of the GUIDs of the objects will be stored in the xref:guid-mapping.adoc[GUID Mapping files].
