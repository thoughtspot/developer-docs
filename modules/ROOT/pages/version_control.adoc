...
* Use one repository per ThoughtSpot `project`. Your ThoughtSpot development, staging, and production environments should all be using the same Git repository. This will make it easier to move objects from `dev` to `prod` (via merging branches).
...
* The branches in Git repository are set up as described in xref:version_control.adoc#_recommended_configuration[Recommended configuration].
...
|`commit_branch_name` [tag greenBackground]#NEW in 9.7.0.cl# |__String__. Name of the remote branch where objects committed from this Thoughtspot instance will be versioned. ThoughtSpot recommends using `commit_branch_name` instead of the `default_branch_name` property. For example, when Orgs are used in a cluster, you can prevent two Orgs from using the same commit branch.
|`default_branch_name` +
...
During this operation, a check is performed between the files in the Git branch and the objects in the ThoughtSpot environment.
...
If the API request is successful, the changes are applied to the objects in the `prod` environment. A tracking file is generated in branch used for storing configuration files. This file includes the `commit_id` specified in the API request.

The subsequent API calls to deploy commits will conisder the saved `commit_id` and `delta_type` specified in the API request:

* If `deploy_type` is set as `DELTA`, all the changes between the last tracked `commit id` and the new `commit_id` specified in the API request will be deployed destination environment or Org.
* If the `deploy_type` is set as `FULL`, all the files from the `commit_id` specified in the API request will be deployed. If any object or file is deleted in the new commit specified in the API request, it will be deleted from the destination environment or Org during deployment.
