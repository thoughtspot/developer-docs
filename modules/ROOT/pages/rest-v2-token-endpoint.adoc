= REST API v2 authentication token endpoints
:toc: true
:toclevels: 3

:page-title: REST API v2 authentication token endpoints
:page-pageid: token-endpoints
:page-description: ThoughtSpot provides different REST API endpoints for generating  authentication token for users

ThoughtSpot provides three endpoints for developers to issue authentication tokens to end users:

* `POST /api/rest/2.0/auth/token/full`
* `POST /api/rest/2.0/auth/token/custom`
* `POST /api/rest/2.0/auth/token/object`

Each endpoint helps developers achieve slightly different use cases:

[cols="1,1,1,3", options="header"]
|===
| Full access token | Custom access token | Object access token | Use cases

| Authenticate users with standard RLS rules and standard object allocation (administered via the user's properties and their group memberships)
| Assign variable values to users when authenticating them, enabling ABAC via RLS for developers (pass RLS values when logging users in ThoughtSpot)
| Assign a subset of objects to a user for a given session. This object list is intersected with the list of objects the user has been given access to via group membership.
|

| Examples
| Authenticate a user via Trusted Auth, without any particular need for RLS
| Authenticate a user, and update its RLS values so that, in tandem with appropriately set up RLS table rules, that user can only access data for specific regions in ThoughtSpot.
| Authenticate a user who usually can see liveboards A, B, and C, but for the context of that session, solely give access to liveboard “A”.
...
* ThoughtSpot recommends developers use the “Full Access Token” method, unless there is a need to set up ABAC via RLS - in which case, the Custom Access Token is recommended.
* For object security, ThoughtSpot recommends using the object security method, rather than the Object Access Token for centralized control of object access.
...
For example: A developer may choose to restrict data access for a user so they only see data for `country="United States"`. This is a row level security rule which should be implemented via the Row Level Security (RLS) feature. If needed, values passed for RLS can be updated via a Custom Access Token.

However, this user may need to be shown data for `state = California`, or `product category = jeans` in ThoughtSpot, so the analysis conducted in ThoughtSpot matches the context of the host application. To accommodate this use case, developers can update runtime/liveboard filters, or parameters via the SDK (see ThoughtSpot's filter overview).

A clear distinction must be made between Row Level Security (RLS) and Context update via HostEvents, runtime filters, parameter updates.
====

[cols="2,2,2", options="header"]
|===
| Row Level Security, and ABAC via RLS through a Custom Access Token | Programmatically set liveboard filter, runtime filter, or parameter values. |
...
Best for use cases that require ABAC via RLS. Create an authentication token that provides values for Row Level Security rules using formula variables, for a given user.

=== Privileges required

* To add a new user and assign privileges during auto creation, you need `ADMINISTRATION` (Can administer ThoughtSpot) privilege.
* If Role-Based Access Control (RBAC) is enabled, the `CONTROL_TRUSTED_AUTH` (Can Enable or Disable Trusted Authentication) privilege and edit access to the data source is required.
* To configure formula variables for all Orgs on your instance or the Primary Org, cluster administration privileges are required.
* Org administrators can configure formula variables for their respective Orgs.
* If RBAC is enabled, users with the `CAN_MANAGE_VARIABLES` (Can manage variables) role privilege can also create and manage variables for their Org context.

=== Usage guidelines

* By default, the token obtained from ThoughtSpot remains valid for 5 mins.
...
=== Attribute-Based Access Control (ABAC) via RLS

To implement ABAC via RLS and assign security entitlements to users during session creation, you can generate a token with custom variable_values. Paired with table-level RLS rules using the formula variables, this allows developers to implement ABAC via RLS. These variable values can be configured to persist for a specific set of models in user sessions initiated with the token, allowing different RLS rules to be set for different data models. Once defined, the rules are added to the user's variable_values object, after which all sessions will use the persisted values.

* Specify the object type as `LOGICAL_TABLE`.
* For more information, see ABAC via RLS Documentation.
...
* The persist_option `RESET` and `NONE` cannot be used when variable_values are provided in the request. If you are working with variable_values, you must use other (`APPEND`, `REPLACE`) supported modes.
...
All options in the token creation APIs that define access to the content in ThoughtSpot will do so during token creation, not when the token is being used for authentication. For example, `auto_create: true` will create the user when the authentication token is created. Persist options such as `APPEND` and `REPLACE` will persist variable_values on the user profile when the token is created.
...
Gets an authentication token and creates a full session in ThoughtSpot for a given user. Best for use cases that do not require ABAC via RLS.

=== Usage guidelines

* By default, the token obtained from ThoughtSpot remains valid for 5 mins.
* You can generate the token for a user by providing a username and password, or by using the cluster’s secret_key (for Trusted authentication).
* To generate a secret_key on your cluster, the administrator must enable Trusted authentication in the Develop > Customizations > Security Settings page.
* When both password and secret_key are included in the API request, password takes precedence.
* If Multi-Factor Authentication (MFA) is enabled on your instance, the API login request with basic authentication (username and password) returns an error. You can switch to token-based authentication with secret_key or contact ThoughtSpot Support for assistance.
...
To add a new user and assign privileges, you need `ADMINISTRATION` (Can administer ThoughtSpot) privilege. If RBAC is enabled, the `CONTROL_TRUSTED_AUTH` (Can Enable or Disable Trusted Authentication) privilege is required.
...
Gets an authentication token that provides access to a specific metadata object. This object list is intersected with the list of objects the user has been given access to via group membership (see Object security).

=== Usage guidelines

* By default, the token obtained from ThoughtSpot remains valid for 5 mins.
* You can generate the token for a user by providing a username and password, or by using the cluster’s secret key (for Trusted authentication).
* To generate a secret_key on your cluster, the administrator must enable Trusted authentication in the Develop > Customizations > Security Settings page.
* When both password and secret_key are included in the API request, password takes precedence.
* If Multi-Factor Authentication (MFA) is enabled on your instance, the API login request with basic authentication (username and password) returns an error. You can switch to token-based authentication with secret_key or contact ThoughtSpot Support for assistance.
